...
public class ConsumerInfoController extends ConsumerAbstractController {
	...
	protected void runStuff(ModelAndView mav, FileBasedFlowManager fm,
		...
		if (fm != null && consumer != null) {
			// Progress Logging...
			CumulatingProgressListener cpl = (CumulatingProgressListener) consumer
			...
			if (cpl != null) {
				...
				eventlist.add("NO CumulatingProgressListener found for "
						+ consumer.getName());
				StatusProgressListener spl = (StatusProgressListener) consumer
						.getProgressListener(StatusProgressListener.class);
				if (spl != null) {
					eventlist.add("Consumer status: " + spl.toString());
				} else {
					// get any pl
					ProgressListener anypl = (ProgressListener) consumer
							.getProgressListener(ProgressListener.class);
					if (anypl != null) {
						eventlist.add("Consumer action task: "
								+ anypl.getTask());
						eventlist.add("Consumer action progress: "
								+ anypl.getProgress() + "%");
					} else {
						eventlist.add("NO ProgressListener found for "
								+ consumer.getName());
					}
				}
			}
			...
			if (action != null) {
				...
				cpl = (CumulatingProgressListener) action
						.getProgressListener(CumulatingProgressListener.class);
				if (cpl != null) {
					eventlist.add("Current action eventlist: "
							+ cpl.getMessages());
				} else {
					StatusProgressListener spl = (StatusProgressListener) action
							.getProgressListener(StatusProgressListener.class);
					if (spl != null) {
						eventlist.add("Current action status: "
								+ spl.toString());
						...
						ProgressListener anypl = (ProgressListener) action
								.getProgressListener(ProgressListener.class);
						if (anypl != null) {
							eventlist.add("Current action task: "
									+ anypl.getTask());
							eventlist.add("Current action progress: "
									+ anypl.getProgress() + "%");
						}
					}
				}
