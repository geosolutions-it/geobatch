...
// extends DefaultFtplet
	...
	private final static Logger LOGGER = Logger.getLogger(GeoBatchFtplet.class
			.getName());
	...
	public FtpletResult onUploadEnd(FtpSession session, FtpRequest request,
		...
		if (reply.getCode() != FtpReply.REPLY_226_CLOSING_DATA_CONNECTION) { // There
			...
			if (LOGGER.isLoggable(Level.INFO))
				LOGGER.log(Level.INFO, "Upload of file '"
						+ request.getArgument() + "' failed.");
			return FtpletResult.DEFAULT;
		} else {
			if (LOGGER.isLoggable(Level.INFO))
				LOGGER.log(Level.INFO, "Upload of file '"
						+ request.getArgument() + "' completed.");
	...
	public FtpletResult onAppendEnd(FtpSession session, FtpRequest request,
		...
		if (reply.getCode() != FtpReply.REPLY_226_CLOSING_DATA_CONNECTION) { // There
																				...
																				// error
			if (LOGGER.isLoggable(Level.INFO))
				LOGGER.log(Level.INFO, "Append of file '"
						+ request.getArgument() + "' failed.");
			return FtpletResult.DEFAULT;
		} else {
			if (LOGGER.isLoggable(Level.INFO))
	...
	protected FtpletResult fireGeoBatchFileAdd(FtpSession session,
			FtpRequest request) throws FtpException, IOException {

		String userPath = session.getUser().getHomeDirectory();
		String currDirPath = session.getFileSystemView().getWorkingDirectory()
				.getAbsolutePath();
		...
		for (FileBasedFlowManager fmloop : catalog
				.getFlowManagers(FileBasedFlowManager.class)) {
			availFmSb.append('(').append(fmloop.getId()).append(',').append(
					fmloop.getName()).append(')');
		...
		if (fm != null) {
			...
			LOGGER.log(Level.INFO,
					"No FlowManager ''{0}'' to notify about {1} -- {2}",
