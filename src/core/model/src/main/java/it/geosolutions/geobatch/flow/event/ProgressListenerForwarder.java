 ...
 * Events are delivered to all the listener in sequence, first to the first
...
public class ProgressListenerForwarder extends ProgressListener implements
	...
	protected final static Logger LOGGER = Logger
			.getLogger(ProgressListenerForwarder.class.toString());
		...
		for (IProgressListener l : listeners) {
			if (LOGGER.isLoggable(Level.FINE))
				LOGGER.fine(getClass().getSimpleName()
						+ " FORWARDING completed message to "
	...
	public void failed(Throwable exception) {
		for (IProgressListener l : listeners) {
			try {
				if (LOGGER.isLoggable(Level.FINE))
					LOGGER.fine(getClass().getSimpleName()
							+ " FORWARDING failed message (" + exception
							+ ") to " + l.getClass().getSimpleName());
				l.failed(exception);
			} catch (Exception e) {
				LOGGER.warning("Exception in event forwarder: " + e);
			}
	...
	public float getProgress() {
		throw new UnsupportedOperationException(
				"Forwarder does not hold a status");
	}
	...
	public void progressing() {
		for (IProgressListener l : listeners) {
			try {
				if (LOGGER.isLoggable(Level.FINE))
					LOGGER.fine(getClass().getSimpleName()
							+ " FORWARDING progressing message to "
							+ l.getClass().getSimpleName());
				l.progressing();
			} catch (Exception e) {
				LOGGER.warning("Exception in event forwarder: " + e);
	...
	public void setProgress(float progress) {
		for (IProgressListener l : listeners) {
			try {
				if (LOGGER.isLoggable(Level.FINE))
					LOGGER.fine(getClass().getSimpleName()
							+ " FORWARDING setProgress message to "
							+ l.getClass().getSimpleName());
	...
	public void terminated() {
		for (IProgressListener l : listeners) {
			if (LOGGER.isLoggable(Level.FINE))
				LOGGER.fine(getClass().getSimpleName()
						+ " FORWARDING terminated message to "
						+ l.getClass().getSimpleName());
			try {
				l.terminated();
			} catch (Exception e) {
