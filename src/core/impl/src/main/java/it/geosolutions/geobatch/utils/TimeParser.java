...
public class TimeParser {
	...
	private static final String[] PATTERNS = { "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'",
			"yyyy-MM-dd'T'HHmmssSSS'Z'", "yyyyMMdd'T'HH:mm:ss.SSS'Z'",
			"yyyyMMdd'T'HHmmssSSS'Z'", "yyyy-MM-dd'T'HH:mm:ss.SSS",
			"yyyy-MM-dd'T'HHmmssSSS", "yyyyMMdd'T'HH:mm:ss.SSS",
			"yyyyMMdd'T'HHmmssSSS",

			"yyyy-MM-dd'T'HH:mm:ss'Z'", "yyyy-MM-dd'T'HHmmss'Z'",
			"yyyyMMdd'T'HH:mm:ss'Z'", "yyyyMMdd'T'HHmmss'Z'",
			"yyyy-MM-dd'T'HH:mm:ss", "yyyy-MM-dd'T'HHmmss",
			"yyyyMMdd'T'HH:mm:ss", "yyyyMMdd'T'HHmmss",

			"yyyy-MM-dd'T'HH:mm'Z'", "yyyy-MM-dd'T'HHmm'Z'",
			"yyyyMMdd'T'HH:mm'Z'", "yyyyMMdd'T'HHmm'Z'", "yyyy-MM-dd'T'HH:mm",
			"yyyy-MM-dd'T'HHmm", "yyyyMMdd'T'HH:mm", "yyyyMMdd'T'HHmm",

			"yyyy-MM-dd'T'HH'Z'", "yyyyMMdd'T'HH'Z'", "yyyy-MM-dd'T'HH",
			"yyyyMMdd'T'HH",
	 ...
	 * ISO-8601 standard. The string may contains either a single date, or a
	 * start time, end time and a period. In the first case, this method returns
	 * a singleton containing only the parsed date. In the second case, this
	 * method returns a list including all dates from start time up to the end
	 * time with the interval specified in the {@code value} string.
	 * 
	 * @param value
	 *            The date, time and period to parse.
	 * @return A list of dates, or an empty list of the {@code value} string is
	 *         null or empty.
	 ...
	 * parameter should follow a syntax defined in the {@link #PATTERNS} array
	 * to be validated.
	 * 
	 * @param value
	 *            The date to parse.
	...
	private Date getDate(final String value) throws ParseException {
		for (int i = 0; i < PATTERNS.length; i++) {
			 ...
			 * because if the parsing stops before the end of the string, the
			 * remaining characters are just ignored and no exception is thrown.
			 * So we have to ensure that the whole string is correct for the
			 * format.
			 */
	 ...
	 * 
	 * @param period
	 *            A string representation of the time increment according the
	 *            ISO-8601:1988(E) standard. For example: {@code "P1D"} = one
	 *            day.
	...
	static long parsePeriod(final String period) throws ParseException {
		...
		if (length != 0 && Character.toUpperCase(period.charAt(0)) != 'P') {
			throw new ParseException("Invalid period increment given: "
					+ period, 0);
		}
		long millis = 0;
		boolean time = false;
		...
		while (++lower < length) {
			...
			while (!Character.isLetter(letter) || letter == 'e'
					|| letter == 'E') {
				if (++upper >= length) {
			...
			final double value = Double.parseDouble(period.substring(lower,
					upper));
			final double factor;
			if (time) {
				switch (letter) {
				...
				case 'W':
					...
					break;
				// TODO: handle months in a better way than just taking the
				// average length.
				case 'M':
				...
				default:
					throw new ParseException(
							"Unknown period symbol: " + letter, upper);
				}
			}
