...
/**
 * Public class to generate lscv08::MERCATOR Model Services 
 * 
 */
public class MERCATORGeneratorService extends
		GeoServerConfiguratorService<FileSystemMonitorEvent, MetocActionConfiguration> {
	
    private final static Logger LOGGER = Logger.getLogger(MERCATORGeneratorService.class.toString());

    /**
     *  Action creator
     * 
     *  @param configuration The data base action configuration 
     *  @return new MERCATORFileConfigurator()
     */
    public MERCATORFileConfigurator createAction(MetocActionConfiguration configuration) {
        try {
            return new MERCATORFileConfigurator(configuration);
        } catch (IOException e) {
