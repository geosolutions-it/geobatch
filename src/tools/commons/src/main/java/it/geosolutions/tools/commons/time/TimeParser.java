 ...
 */
package it.geosolutions.tools.commons.time;
...
public class TimeParser {
    ...
    private static final String[] PATTERNS = { "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'",
            ...
            "yyyy-MM-dd'T'HH:mm:ss'Z'", "yyyy-MM-dd'T'HHmmss'Z'", "yyyyMMdd'T'HH:mm:ss'Z'",
            "yyyyMMdd'T'HHmmss'Z'", "yyyy-MM-dd'T'HH:mm:ss", "yyyy-MM-dd'T'HHmmss",
            "yyyyMMdd'T'HH:mm:ss", "yyyyMMdd'T'HHmmss",

            "yyyy-MM-dd'T'HH:mm'Z'", "yyyy-MM-dd'T'HHmm'Z'", "yyyyMMdd'T'HH:mm'Z'",
            "yyyyMMdd'T'HHmm'Z'", "yyyy-MM-dd'T'HH:mm", "yyyy-MM-dd'T'HHmm", "yyyyMMdd'T'HH:mm",
            "yyyyMMdd'T'HHmm",

            "yyyy-MM-dd'T'HH'Z'", "yyyyMMdd'T'HH'Z'", "yyyy-MM-dd'T'HH", "yyyyMMdd'T'HH",
     ...
     */
    public static List<Date> parse(String value) throws ParseException {
        if (value == null) {
    ...
    private static Date getDate(final String value) throws ParseException {
        for (int i = 0; i < PATTERNS.length; i++) {
            ...
            SimpleDateFormat format = new SimpleDateFormat(PATTERNS[i],Locale.UK);
            format.setLenient(true);
            format.setCalendar(Calendar.getInstance(UTC_TZ));

            /*
             * We do not use the standard method DateFormat.parse(String), because if the parsing
             * stops before the end of the string, the remaining characters are just ignored and no
             * exception is thrown. So we have to ensure that the whole string is correct for the
             * format.
             */
