...
import it.geosolutions.tools.commons.reader.CSVReaderTest;
...
public class CSVWriterTest {
    Logger LOGGER = LoggerFactory.getLogger(CSVWriterTest.class);

    File csv, csvOut;

    @Before
    public void setUp() throws Exception {
        csv = TestData.file(CSVReaderTest.class, "test.csv");
        csvOut = org.geotools.TestData.temp(CSVReaderTest.class, "withErrorData.csv");
    }
    ...
    public void toMapTest() throws IllegalArgumentException, IOException {
        ...
        CSVWriter.writeCsv(LOGGER, data.descendingSet(), csvOut, ",", false);

        TreeSet<Object[]> newData = CSVReader.readCsv(LOGGER, csvOut, ",", new SetComparator<Integer>(new IntegerCaster(),2,true), false, false);
        
        newData.first();// matches 23
        Assert.assertNotNull(first);
        Assert.assertNotNull(first[2].equals("23"));
        Assert.assertTrue(first[3].equals("23_airtmp_mn.tif"));
