...
public abstract class BaseFileSystemMonitor implements FileSystemMonitor {
	...
	private final static Logger LOGGER = Logger
			.getLogger(BaseFileSystemMonitor.class.toString());
	...
	private class EventManager implements Runnable {
		...
		/**
		 * Sending an event by putting it inside the Swing dispatching thred.
		 * This might be useless in command line app but it is important in GUi
		 * apps. I might change this though.
		 * 
		 * @param file
		 */
		...
		public void run() {
			try {
				...
				LOGGER.log(Level.SEVERE, new StringBuffer(
						"Caught an IOException Exception: ").append(
	...
	protected BaseFileSystemMonitor(File file, String wildCard,
		...
		if (file == null)
			throw new IllegalArgumentException(
					"Null file provided to this FileSystemMonitor");
		if (wildCard != null && wildCard.length() <= 0)
			throw new IllegalArgumentException("Empty wild card provided");
		if (wildCard != null && wildCard.length() > 0
				&& (file.exists() && !file.isDirectory()))
			throw new IllegalArgumentException(
					"Wild card provided, while monitoring a singl file");
		this.file = file;
	...
	public synchronized void removeListener(
			FileSystemMonitorListener fileListener) {
