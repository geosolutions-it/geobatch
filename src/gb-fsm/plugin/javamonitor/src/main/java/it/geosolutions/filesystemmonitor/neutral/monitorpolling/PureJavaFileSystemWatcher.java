...
public final class PureJavaFileSystemWatcher extends BaseFileSystemMonitor {
     ...
     */
    public PureJavaFileSystemWatcher(File file, String wildCard, long pollingInterval) {
        super(file, wildCard);
        ...
        if (this.file.exists() && file.canRead()) {
            ...
            if (wildCard == null) {
                ...
                files = this.file.listFiles((FileFilter) new WildcardFileFilter(wildCardString,
                        IOCase.INSENSITIVE));
            else
                throw new IllegalStateException("Cannot use a wildcard with aplain file");

            for (int i = 0; i < files.length; i++) {
                filesMap.put(files[i].getName(), new Long(files[i].lastModified()));// name -- last
                                                                                    // modified
        ...
        this.workers = new Thread[] { new PureJavaMonitorNotifier(pollingInterval) };
        ...
        synchronized (filesMap) {
            ...
            if (newModifiedTime != lastModifiedTime) {
                ...
                // //
                    ...
                    final String prefix = new StringBuffer(file.getAbsolutePath().trim()).append(
                            File.separator).toString();

                    // get updated file list
                    final String[] newFiles = wildCardString != null ? file
                            .list((FilenameFilter) new WildcardFileFilter(wildCardString,
                                    IOCase.INSENSITIVE)) : file.list();
    ...
    public synchronized void pause() {
        if (this.workers != null) {
            for (int i = 0; i < workers.length; i++) {
                if (this.workers[i] != null) {
                    ((AbstractPausableThread) this.workers[i]).setPauseRequested(true);
                }
            }
        ...
        if (this.workers != null) {
            for (int i = 0; i < workers.length; i++) {
                if (this.workers[i] != null) {
                    ...
                    ((AbstractPausableThread) this.workers[i]).setTerminationRequested(false);
                    ((AbstractPausableThread) this.workers[i]).setPaused(false);
                }
            }
