...
public class EOProcessor {
    ...
    public EarthObservationPackage parsePackage() {
        try {
            ...
            if (pckList == null) {
                ...
                throw new RuntimeException("Expecting one package file in " + dir + ", but found "
                        + pckList.length);
            }
            File packFile = pckList[0];
            pkg = new EarthObservationPackage(packFile);

            if (pkg.getEoFileName() != null) {
    ...
    public SARProductRO getSARProduct() {
        return eo2sar(eo, getGmlBaseURI());
    }
    ...
    public static SARProductRO eo2sar(EarthObservation eo, String baseURI) {
        ...
        sar.setGmlBaseURI(baseURI);
        sar.setGmlFileName(eo.getGmlFileName());
    ...
    /**
     * @param gmlBaseURI the gmlBaseURI to set
     */
