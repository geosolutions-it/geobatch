...
public class ConfigurationDeserializationTest {
	...
	@Test
	public void testGeoServerShapeActionConfigurationDeserialization() throws Exception{
		XStream xstream = new XStream();
		Alias alias=new Alias();
		alias.setAliasRegistry(aliasRegistry);
		alias.setAliases(xstream);
		File configFile = new File("src/test/resources/shapefileFlow.xml");
		...
		for(ActionConfiguration actionConfiguration : configuration.getEventConsumerConfiguration().getActions()){
			if(actionConfiguration != null && actionConfiguration instanceof GeoServerShapeActionConfiguration){
