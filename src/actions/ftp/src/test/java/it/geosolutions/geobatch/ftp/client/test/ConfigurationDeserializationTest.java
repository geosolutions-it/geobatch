...
public class ConfigurationDeserializationTest {
	...
	public void testDeserialization() throws Exception{
		...
		File configFile = new File("src/test/resources/ftpflow.xml");
		...
		for(ActionConfiguration actionConfiguration : configuration.getEventConsumerConfiguration().getActions()){
			if(actionConfiguration != null && actionConfiguration instanceof FTPActionConfiguration){
				FTPActionConfiguration ftpAction = (FTPActionConfiguration)actionConfiguration;
				if(ftpAction.getOperationId() != null){
					configurationDeserialized = true;
				}
				break;
			}
		}
		assert(configurationDeserialized);
