package it.geosolutions.geobatch.ftp.client.test;
...
import it.geosolutions.geobatch.ftp.client.configuration.FTPAction;
...
public class AnnotationProcessingTest {
	...
	@Test
	public void testFTPDeleteConfigurationAliased() throws Exception{
		Boolean configurationAliasRegistered = false;
		Class configurationClass = FTPActionConfiguration.class;
		Iterator<Entry<String, Class<?>>> it = aliasRegistry.iterator();
		while (it.hasNext()){
			Entry<String, Class<?>> alias = it.next();	
			
			Action action = (Action)FTPAction.class.getAnnotations()[0];
	...
	@Test
	public void testFTPActionServiceGeneration() throws Exception{
		...
		for(GenericActionService action : actionList){
			if(FTPActionConfiguration.class.getSimpleName().equals(action.getId())){
