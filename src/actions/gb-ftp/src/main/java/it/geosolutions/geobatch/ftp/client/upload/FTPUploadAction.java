...
/**
 * This class represent an extended FTP action to upload local files or
 * directory.
 * 
...
public class FTPUploadAction extends FTPBaseAction<FileSystemMonitorEvent> {
	...
	public Queue<FileSystemMonitorEvent> execute(
		...
		try {
			...
			if (zipMe) {
				...
				final File zippedFile = IOUtils.zip(tempDir, zipFileName,
						filesToSend.toArray(new File[filesToSend.size()]));

				FTPHelperBare.putBinaryFileTo(ftpserverHost, zippedFile
						.getAbsolutePath(), path, ftpserverUSR, ftpserverPWD,
						ftpserverPort, WriteMode.OVERWRITE, connectMode,
						timeout);
			} else {
				...
				for (File file : filesToSend) {
					if (file.isFile()) {
						FTPHelperBare.putBinaryFileTo(ftpserverHost, file
								.getAbsolutePath(), path, ftpserverUSR,
								ftpserverPWD, ftpserverPort,
			...
			if (LOGGER.isLoggable(Level.INFO))
				LOGGER
						.info("FTPUploadAction: file SUCCESSFULLY sent to FtpServer!");
			// else
			// if (LOGGER.isLoggable(Level.INFO))
	 ...
	 */
	private void putDirectory(final File file, final String path)
		...
		for (int i = 0, n = files.length; i < n; i++) {
			if (files[i].isDirectory()) {
				...
				FTPHelperBare.putBinaryFileTo(ftpserverHost, files[i]
						.getAbsolutePath(), dirPath, ftpserverUSR,
