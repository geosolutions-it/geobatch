 ...
 * 
 * Based on FTPHelper, here methods will not catch exceptions, that will be
 * rethrown.
 * 
...
public class FTPHelperBare {
	...
	private static final Logger LOGGER = Logger.getLogger(FTPHelperBare.class
			.toString());
	 ...
	 */
	public static void putBinaryFileTo(String ftpserverHost, String binaryFile,
			String path, String ftpserverUser, String ftpserverPassword,
			int ftpserverPort, WriteMode writeMode, FTPConnectMode connectMode,
		...
		try {
			putFile(ftpserverHost, binaryFile, path, ftpserverUser,
					ftpserverPassword, ftpserverPort, FTPTransferType.BINARY,
					writeMode, connectMode, timeout);
		} finally {
			if (LOGGER.isLoggable(Level.INFO))
	 ...
	 */
	private static FTPClient connectTo(String ftpserverHost,
			String ftpserverUser, String ftpserverPassword, int ftpserverPort,
			FTPTransferType transferType, WriteMode writeMode,
			FTPConnectMode connectMode, int timeout) throws FTPException,
			IOException {
	 ...
	 */
	private static void putFile(String ftpserverHost, String binaryFile,
			String path, String ftpserverUser, String ftpserverPassword,
			int ftpserverPort, FTPTransferType transferType,
		...
		try {
			ftp = connectTo(ftpserverHost, ftpserverUser, ftpserverPassword,
					ftpserverPort, transferType, writeMode, connectMode,
					timeout);
			...
			remoteFileName = remoteFileName.substring(remoteFileName
					.lastIndexOf("/") + 1, remoteFileName.length());

			if (LOGGER.isLoggable(Level.INFO))
				LOGGER.info("[FTP::PutFileTo]: " + "Connecting to :"
						+ ftpserverHost + ":" + ftpserverPort);

			if (LOGGER.isLoggable(Level.INFO))
				LOGGER.info("[FTP::FileTo]: " + "sending: " + binaryFile
						+ " to: " + remoteFileName);
			...
			final String remoteFileNameReturned = ftp.put(binaryFile,
					remoteFileName);
			...
			if (remoteFileName == null)
				throw new RuntimeException("PUT failed for file "
						+ remoteFileName);
	 ...
	 * @return If true the upload has been successful.
	 */
	public static void putTextFileTo(String ftpserverHost, String textFile,
			String path, String ftpserverUser, String ftpserverPassword,
			int ftpserverPort, WriteMode writeMode, FTPConnectMode connectMode,
			int timeout) throws IOException, FTPException {
		if (LOGGER.isLoggable(Level.INFO))
		...
		try {
			putFile(ftpserverHost, textFile, path, ftpserverUser,
					ftpserverPassword, ftpserverPort, FTPTransferType.ASCII,
					writeMode, connectMode, timeout);
		} finally {
			if (LOGGER.isLoggable(Level.INFO))
				LOGGER.info("[FTP::PutFileTo]: " + "end");
	 ...
	 */
	public static void createDirectory(String ftpserverHost, String binaryFile,
			String path, String ftpserverUser, String ftpserverPassword,
			int ftpserverPort, FTPTransferType transferType,
			WriteMode writeMode, FTPConnectMode connectMode, int timeout)
			throws IOException, FTPException {
		...
		try {
			...
			if (LOGGER.isLoggable(Level.INFO))
				LOGGER.info("[FTP::createDirectory]: " + "Connecting to :"
						+ ftpserverHost + ":" + ftpserverPort);

			if (LOGGER.isLoggable(Level.INFO))
				LOGGER.info("[FTP::createDirectory]: " + "sending: "
						+ binaryFile + " to: " + remoteFileName);
	 ...
	 */
	public static void downloadFile(String ftpserverHost, String localPath,
			String remotePath, String remoteFile, String ftpserverUser,
			String ftpserverPassword, int ftpserverPort,
			FTPTransferType transferType, WriteMode writeMode,
		...
		try {
			...
			if (LOGGER.isLoggable(Level.INFO))
				LOGGER.info("[FTP::downloadFile]: " + "Connecting to :"
						+ ftpserverHost + ":" + ftpserverPort);

			if (LOGGER.isLoggable(Level.INFO))
				LOGGER.info("[FTP::downloadFile]: " + "downloading: "
			...
			if (LOGGER.isLoggable(Level.INFO))
				LOGGER.info("[FTP::downloadFile]: " + "downloaded: "
						+ remoteFile + " from: " + remoteFile);
	 ...
	 */
	public static FTPFile[] dirDetails(String ftpserverHost, String dirName,
			String remotePath, String ftpserverUser, String ftpserverPassword,
			int ftpserverPort, FTPTransferType transferType,
			WriteMode writeMode, FTPConnectMode connectMode, int timeout)
		...
		try {
			...
			if (LOGGER.isLoggable(Level.INFO))
				LOGGER.info("[FTP::dirDetails]: " + "Connecting to :"
						+ ftpserverHost + ":" + ftpserverPort);
	 ...
	 * @param timeout
	 *            The connection timeout.
	 * @return If true the remote file or directory has been successfully
	 *         deleted.
	 */
	public static void deleteFileOrDirectory(String ftpserverHost,
			String remoteFile, boolean isDir, String remotePath,
			String ftpserverUser, String ftpserverPassword, int ftpserverPort,
			FTPConnectMode connectMode, int timeout) throws FTPException,
			IOException {
		...
		try {
			...
			if (LOGGER.isLoggable(Level.INFO))
				LOGGER.info("[FTP::deleteFileOrDirectory]: "
						+ "Connecting to :" + host + ":" + port);

			if (LOGGER.isLoggable(Level.INFO))
				LOGGER.info("[FTP::deleteFileOrDirectory]: " + "removing: "
						+ remoteFile + " from: " + remoteFile);
			...
			if (LOGGER.isLoggable(Level.INFO))
				LOGGER.info("[FTP::deleteFileOrDirectory]: " + "removed: "
						+ remoteFile + " from: " + remoteFile);
