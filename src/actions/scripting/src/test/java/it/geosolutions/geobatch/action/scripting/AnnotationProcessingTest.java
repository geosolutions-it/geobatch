...
public class AnnotationProcessingTest {
	...
	@Test
	public void testScriptingConfigurationAliased() throws Exception{

		Boolean configurationAliasRegistered = false;
		Class configurationClass = ScriptingConfiguration.class;
		Iterator<Entry<String, Class<?>>> it = aliasRegistry.iterator();
		while (it.hasNext()){
			Entry<String, Class<?>> alias = it.next();	

			Action action = (Action)ScriptingAction.class.getAnnotations()[0];
	...
	@Test
	public void testScriptingActionServiceGeneration() throws Exception{
		...
		for(GenericActionService action : actionList){
			if(ScriptingConfiguration.class.getSimpleName().equals(action.getId())){
