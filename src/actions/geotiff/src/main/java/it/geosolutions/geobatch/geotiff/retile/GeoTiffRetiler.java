...
public class GeoTiffRetiler extends BaseAction<FileSystemMonitorEvent>
	...
	public Queue<FileSystemMonitorEvent> execute(
		...
		try {
			...
			final File tiledInputFile = new File(inputFile.getParent(), name
					+ "_tiled." + extension);
			...
			final GridCoverage2D inCoverage = (GridCoverage2D) reader
					.read(null);
			...
			final AbstractGridCoverageWriter writer = (AbstractGridCoverageWriter) new GeoTiffWriter(
					outFile);
			writer.write(inCoverage, (GeneralParameterValue[]) wparams.values()
