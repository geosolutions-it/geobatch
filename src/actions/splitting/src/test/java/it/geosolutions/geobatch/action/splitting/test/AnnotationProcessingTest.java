...
public class AnnotationProcessingTest {
	...
	@Test
	public void testSplittingConfigurationAliased() throws Exception{

		Boolean configurationAliasRegistered = false;
		Class configurationClass = SplittingConfiguration.class;
		Iterator<Entry<String, Class<?>>> it = aliasRegistry.iterator();
		while (it.hasNext()){
			Entry<String, Class<?>> alias = it.next();	

			Action action = (Action)SplittingAction.class.getAnnotations()[0];
	...
	@Test
	public void testSplittingActionServiceGeneration() throws Exception{
		...
		for(GenericActionService action : actionList){
			if(SplittingConfiguration.class.getSimpleName().equals(action.getId())){
