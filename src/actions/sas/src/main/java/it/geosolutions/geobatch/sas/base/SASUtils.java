...
public class SASUtils {
	...
	public static String buildWmsPath(final String name) {
		if (name==null || name.trim().length()==0)
			return "";
        SASDirNameParser parser = SASDirNameParser.parse(name);
        if(parser == null)
			return "";

        final StringBuilder wmsPath = new StringBuilder("/")
                .append(parser.getDate()).append("/")
                .append(parser.getMission()).append("/")
                .append(parser.getLeg()).append("/")
                .append(parser.getChannel())
                ;
        return wmsPath.toString();

//		final int missionIndex = name.indexOf("_");
//        final String timePrefix = name.substring(0,missionIndex);
//        final int legIndex = name.indexOf(SASUtils.LEG_PREFIX);
