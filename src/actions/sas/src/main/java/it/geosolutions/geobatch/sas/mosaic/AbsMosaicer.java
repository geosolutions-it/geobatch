 ...
 */
public abstract class AbsMosaicer
    ...
    private class OverviewsEmbedderTask implements Callable<String> {
		...
		public String call() throws Exception {
    		try{
    			SASUtils.addOverviews(fileName, gtovConfiguration);
	    		return fileName;
    		}finally{
        		// decrement latch
		...
		public String call() throws Exception {
	        ...
	        try {
	            ...
	            if (LOGGER.isLoggable(Level.INFO))
	            	LOGGER.info(new StringBuilder("Writing tile: ")
                            .append(column+1).append(" of ").append(numTileX).append(" [X] -- ")
                            .append(row+1).append(" of ").append(numTileY).append(" [Y]")
                            .toString());
	            ...
	            writerWI.write(gc, (GeneralParameterValue[]) params.values().toArray(new GeneralParameterValue[1]));
	        	 ...
	        	 IOException ex = new IOException("Exception occurred whilst writing tiles");
                 ex.initCause(e);
                 throw ex;
	        }finally{
	        	if(writerWI!=null){
