 ...
 * 
 */
public class MERCATORFileConfiguratorAction extends
	...
	protected void writeDownNetCDF(File outDir, String inputFileName)
		...
		final List<Dimension> outDimensions = METOCSActionsIOUtils
				.createNetCDFCFGeodeticDimensions(ncFileOut, true, 1, true,
						depth_dim.getLength(), METOCSActionsIOUtils.DOWN, true,
		...
		for (String varName : foundVariables.keySet()) {
			...
			final SampleModel outSampleModel = Utilities
					.getSampleModel(var.getDataType(), lon_dim.getLength(),
							lat_dim.getLength(), 1);
			...
			for (int z = 0; z < depth_dim.getLength(); z++) {
				...
				for (int y = 0; y < lat_dim.getLength(); y++)
					for (int x = 0; x < lon_dim.getLength(); x++)
						outVarData.setFloat(outVarData.getIndex().set(0, z, y,
