...
public class NURCWPSOutput2WMCFileAction extends
	...
	public Queue<FileSystemMonitorEvent> execute(
		...
		try {
					...
					if (extension.equalsIgnoreCase("nc")
						...
						try {
							...
							if (!initialized) {
								...
								ncFileOut.write(METOCSActionsIOUtils.LAT_DIM,
										latOriginalData);
								ncFileOut.write(METOCSActionsIOUtils.LON_DIM,
										lonOriginalData);
								ncFileOut.write(METOCSActionsIOUtils.TIME_DIM,
										timeData);
							}
			...
			if (latDim == null)
				latDim = ncFileIn
						.findDimension(METOCSActionsIOUtils.LAT_DIM_LONG);
			...
			if (lonDim == null)
				lonDim = ncFileIn
						.findDimension(METOCSActionsIOUtils.LON_DIM_LONG);
			...
			final Variable yearOriginalVar = ncFileIn
					.findVariable(METOCSActionsIOUtils.TIME_YEAR);
			...
			final Variable dayOriginalVar = ncFileIn
					.findVariable(METOCSActionsIOUtils.TIME_DAY);
			final Variable hourOriginalVar = ncFileIn
					.findVariable(METOCSActionsIOUtils.TIME_HOUR);
			...
			if (!hasTime) {
				...
				if (isTDA) {
					baseTime = sdf.format(timeOriginalData
							...
							* 1000 + METOCSActionsIOUtils.startTime);
				} else {
			...
			for (Variable var : foundVariables) {
				if (var != null) {
					...
					if (var.getRank() == 1
							|| varName
									.equalsIgnoreCase(METOCSActionsIOUtils.LAT_DIM)
							|| varName
									.equalsIgnoreCase(METOCSActionsIOUtils.LON_DIM)
							|| varName
									.equalsIgnoreCase(METOCSActionsIOUtils.LAT_DIM_LONG)
							|| varName
									.equalsIgnoreCase(METOCSActionsIOUtils.LON_DIM_LONG)
							|| varName
					...
					if (canProceed) {
						...
						for (int t = 0; t < (hasLocalTime ? nTime : 1); t++) {
							for (int z = 0; z < (hasLocalZLevel ? nZeta : 1); z++) {
								...
								final Array minMaxArray = METOCSActionsIOUtils
