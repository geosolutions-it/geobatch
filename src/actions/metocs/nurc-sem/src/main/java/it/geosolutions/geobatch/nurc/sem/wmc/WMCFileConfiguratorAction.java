        ...
        try {
            ...
            while (events.size() > 0) {
                ...
                final String path = new File(inputFile.getParentFile(), props.getProperty("path"))
                        .getAbsolutePath();
                final String infoFile = new StringBuilder(inputFileName.substring(0, inputFileName
                        .length() - 6)).append(INFO_EXTENSION).toString();
            ...
            geoserverUrl = (geoserverUrl.contains("wms") ? geoserverUrl : geoserverUrl
            ...
            final File outputDir = configuration.getOutputDirectory() != null ? new File(
                    configuration.getOutputDirectory()) : workingDir;
    ...
    private void setAdditionalInfo(final String statisticFile, final WMCExtension extension,
        ...
        if (file.exists()) {
            ...
            try {
                ...
                extension.setStyleMinValue(new OLStyleMinValue(minStyle.substring(0, minStyle
                        .length() - 1), minDef));
                extension.setStyleMaxValue(new OLStyleMaxValue(maxStyle.substring(0, maxStyle
                        .length() - 1), maxDef));
                minMaxSet = true;
            } finally {
                if (reader != null) {
