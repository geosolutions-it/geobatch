...
import it.geosolutions.geobatch.nurc.sem.wmc.model.OLTransparent;
import it.geosolutions.geobatch.nurc.sem.wmc.model.ViewContext;
import it.geosolutions.geobatch.nurc.sem.wmc.model.WMCBoundingBox;
import it.geosolutions.geobatch.nurc.sem.wmc.model.WMCExtension;
import it.geosolutions.geobatch.nurc.sem.wmc.model.WMCFormat;
import it.geosolutions.geobatch.nurc.sem.wmc.model.WMCLayer;
...
public class WMCFileConfiguratorAction extends
		BaseAction<FileSystemMonitorEvent> implements
		Action<FileSystemMonitorEvent> {

	private final static Logger LOGGER = Logger
			.getLogger(WMCFileConfiguratorAction.class.toString());
	...
	protected WMCFileConfiguratorAction(WMCActionConfiguration configuration)
			throws IOException {
		super(configuration);
	...
	public Queue<FileSystemMonitorEvent> execute(
		...
		try {
			...
			while (events.size() > 0) {
				...
				final String infoFile = new StringBuilder(inputFileName
						.substring(0, inputFileName.length() - 6)).append(
						INFO_EXTENSION).toString();

				final AbstractGridCoverage2DReader reader = ((AbstractGridFormat) acquireFormat(driver))
						.getReader(new File(path).toURI().toURL());
				...
				if (metadataNames != null && metadataNames.length > 0) {
					...
					// ELEVATION DIMENSION
					elevationMetadata = reader
							.getMetadataValue("ELEVATION_DOMAIN");
				}
				...
				if (elevationMetadata != null) {
					final String[] elevationLevels = elevationMetadata
							.split(",");
			...
			geoserverUrl = (geoserverUrl.contains("wms") ? geoserverUrl
					: geoserverUrl
							+ (geoserverUrl.endsWith("/") ? "wms" : "/wms"));
			...
			if (configuration.getBaseLayerId() != null) {
				...
				final String baseLayerFormat = configuration
						.getBaseLayerFormat();

				WMCLayer baseLayer = new WMCLayer("0", "0", baseLayerName,
						baseLayerTitle, crs);
			...
			for (WMCEntry entry : entryList) {
				...
				WMCLayer newLayer = new WMCLayer("0", "1", nameSpace + ":"
						+ layerName, layerTitle, crs);
				WMCServer server = new WMCServer("wms", "1.1.1", "wms");
				List<WMCFormat> formatList = new ArrayList<WMCFormat>();
				// List<WMCStyle> styleList = new ArrayList<WMCStyle>();
				WMCExtension extension = new WMCExtension();
				...
				extension
						.setDisplayInLayerSwitcher(new OLDisplayInLayerSwitcher(
								"false"));
				OLStyleColorRamps ramp = new OLStyleColorRamps(
						"jet,red,blue,gray");
				...
				extension.setStyleRestService(new OLStyleRestService(
						configuration.getGeoserverURL() + "/rest/sldservice/"
								+ nameSpace + ":" + layerName
								+ "/rasterize.sld"));

				if (entry.getDimensions() != null) {
					for (String dim : entry.getDimensions().keySet()) {
						final String values = entry.getDimensions().get(dim)
								.get("values");
						final String defaultValue = entry.getDimensions().get(
								dim).get("default");

						if ("TIME".equals(dim)) {
							extension.setTime(new OLDimension(values, dim,
									defaultValue));
						} else if ("ELEVATION".equals(dim)) {
							extension.setElevation(new OLDimension(values, dim,
									defaultValue));
						}
					}
				}
				...
				// styleList.add(new WMCStyle("1", new WMCSLD(new
				// WMCOnlineResource("simple",
			...
			final File outputDir = configuration.getOutputDirectory() != null ? new File(
			...
			if (outputDir != null && outputDir.exists()
				...
				try {
					outFile = new FileWriter(new File(outputDir, "WMC_"
							+ sessionId + ".xml"));
	...
	private void setAdditionalInfo(final String statisticFile,
			final WMCExtension extension, final WMCLayer newLayer)
			throws IOException {
		...
		if (file.exists()) {
			...
			try {
				...
				extension.setStyleMinValue(new OLStyleMinValue(minStyle
						.substring(0, minStyle.length() - 1), minDef));
				extension.setStyleMaxValue(new OLStyleMaxValue(maxStyle
						.substring(0, maxStyle.length() - 1), maxDef));
				minMaxSet = true;
			} finally {
				if (reader != null) {
					try {
