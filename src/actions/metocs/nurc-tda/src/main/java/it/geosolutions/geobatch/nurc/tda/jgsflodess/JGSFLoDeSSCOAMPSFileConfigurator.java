...
public class JGSFLoDeSSCOAMPSFileConfigurator extends MetocConfigurationAction<FileSystemMonitorEvent> {
    ...
    public Queue<FileSystemMonitorEvent> execute(Queue<FileSystemMonitorEvent> events)
        ...
        try {
            ...
            final File coampsDatasetDirectory = Utilities.decompress("COAMPS", event.getSource(),
                    tempFile);
            ...
            // ////
            final File outputFile = new File(outDir, "JGSFLoDeSS_COAMPS-Forecast-T"
                    + new Date().getTime()
            ...
            for (File COAMPSFile : COAMPSFiles) {
                ...
                if (COAMPSFileGrid != null && COAMPSFileGrid.getWidth() == width
                        && COAMPSFileGrid.getHeight() == height) {
                    final String timeInstant = COAMPSFileGrid.getTimeGroup().substring(0,
                            COAMPSFileGrid.getTimeGroup().length() - 2)
                            + "_" + COAMPSFileGrid.getForecastTime().substring(1);
                    ...
                    if (!varsFound.contains(COAMPSFileGrid.getParamName().replaceAll("_", ""))) {
                        ...
                        for (MetocElementType m : metocDictionary.getMetoc()) {
                            if ((varName.equalsIgnoreCase("lndsea") && m.getName().equals(
                                    "land or sea"))
                                    || (varName.equalsIgnoreCase("ustrue") && m.getName().equals(
                                            "wind stress u-component"))
                                    || (varName.equalsIgnoreCase("vstrue") && m.getName().equals(
            ...
            int t0 = Integer
                    .parseInt(timesFound.get(0).substring(timesFound.get(0).lastIndexOf("_") + 1,
                            timesFound.get(0).lastIndexOf("_") + 4));
            int t1 = (timesFound.size() > 0 ? Integer
                    .parseInt(timesFound.get(1).substring(timesFound.get(1).lastIndexOf("_") + 1,
            ...
            final List<Dimension> outDimensions = METOCSActionsIOUtils
                    .createNetCDFCFGeodeticDimensions(ncFileOut, true, timesFound.size(), true,
                            levelsFound.size(), METOCSActionsIOUtils.UP, true, height, true, width);
            ...
            for (int y = 0; y < height; y++)
                lat1Data.setFloat(lat1Data.getIndex().set(y), (float) (envelope.getMinimum(1) + y
                        * resY));
            ...
            for (int x = 0; x < width; x++)
                lon1Data.setFloat(lon1Data.getIndex().set(x), (float) (envelope.getMinimum(0) + x
                        * resX));
            ...
            for (File COAMPSFile : COAMPSFiles) {
                ...
                if (COAMPSFileGrid != null && COAMPSFileGrid.getWidth() == width
                    ...
                    final Variable outVar = ncFileOut.findVariable(foundVariableBriefNames
                            .get(varName));
                    ...
                    for (int z = 0; z < levelsFound.size(); z++)
                        for (int y = 0; y < height; y++)
                            for (int x = 0; x < width; x++) {
                                ...
                                outVarData.setFloat(outVarData.getIndex().set(tIndex, z, y, x),
                                        sample);
                            }
