		...
		try {
			...
			final List<Dimension> outDimensions = METOCSActionsIOUtils
					.createNetCDFCFGeodeticDimensions(ncFileOut, true,
							...
							METOCSActionsIOUtils.UP, true, height, true, width);
			...
			for (int z = 0; z < levelsFound.size(); z++)
				zeta1Data.setFloat(zeta1Data.getIndex().set(z), levelsFound
						.get(z));
			ncFileOut.write(METOCSActionsIOUtils.HEIGHT_DIM, zeta1Data);
			...
			for (File COAMPSFile : COAMPSFiles) {
				...
				if (COAMPSFileGrid != null
					...
					METOCSActionsIOUtils.write2DData(userRaster, COAMPSFileGrid,
							false, false);

					// Resampling to a Regular Grid ...
					if (LOGGER.isLoggable(Level.FINE))
						LOGGER.fine("Resampling to a Regular Grid ...");
					userRaster = METOCSActionsIOUtils.warping(COAMPSFileGrid,
