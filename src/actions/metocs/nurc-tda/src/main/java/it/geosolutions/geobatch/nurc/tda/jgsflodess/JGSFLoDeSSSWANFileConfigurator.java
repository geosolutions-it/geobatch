		...
		try {
			...
			final List<Dimension> outDimensions = METOCSActionsIOUtils
					.createNetCDFCFGeodeticDimensions(ncFileOut, true, timeDim
							...
							METOCSActionsIOUtils.UP, true, latDim.getLength(),
			...
			for (String varName : foundVariables.keySet()) {
				...
				if (variableBrief != null) {
					...
					if (!Double.isNaN(fillValue)) {
						outNoData = (float) METOCSActionsIOUtils.rescaleValue(
			...
			for (Object object : ncFileIn.getVariables()) {
				...
				if (var != null) {
					...
					if (!Double.isNaN(fV)) {
						outNoData = (float) METOCSActionsIOUtils.rescaleValue(fV,
								scale, offset);
									...
									destArray
											.setDouble(
													destArray.getIndex().set(t,
															z, y, x),
													(float) ((originalValue != noData && originalValue != fillValue) ? METOCSActionsIOUtils
															.rescaleValue(
																	originalValue,
																	scale,
																	offset)
