...
public class JGSFLoDeSSSWANFileConfigurator extends MetocConfigurationAction<FileSystemMonitorEvent> {
    ...
    public Queue<FileSystemMonitorEvent> execute(Queue<FileSystemMonitorEvent> events)
        ...
        try {
            ...
            // ////
            final File outputFile = new File(outDir, "JGSFLoDeSS_SWAN-Forecast-T"
                    + new Date().getTime()
            ...
            for (int t = 0; t < nTimes; t++) {
                ...
                if (t == 0 && nTimes > 1) {
                    TAU = (int) Math.ceil((timeOriginalData.getLong(timeOriginalData.getIndex()
                            .set(t + 1)) - timeValue) / 3600.0);
                }
                // adding time offset
            ...
            final List<Dimension> outDimensions = METOCSActionsIOUtils
                    .createNetCDFCFGeodeticDimensions(ncFileOut, true, timeDim.getLength(),
                            hasZeta, nZeta, METOCSActionsIOUtils.UP, true, latDim.getLength(),
                ...
                if (var != null) {
                    ...
                    if (foundVariables.get(varName) == null) {
                        ...
                        for (MetocElementType m : metocDictionary.getMetoc()) {
                            if ((varName.equalsIgnoreCase("sig_wav_ht") && m.getName().equals(
                                    "significant wave height"))
                                    || (varName.equalsIgnoreCase("peak_wav_per") && m.getName()
                                            .equals("peak wave period"))
                                    || (varName.equalsIgnoreCase("mean_wav_dir") && m.getName()
                                            .equals("mean wave direction"))) {
                                longName = m.getName();
            ...
            for (String varName : foundVariables.keySet()) {
                ...
                if (variableBrief != null) {
                    ...
                    if (!Double.isNaN(fillValue)) {
                        outNoData = (float) METOCSActionsIOUtils.rescaleValue(fillValue, scale,
                                offset);
            ...
            NetCDFConverterUtilities.setData1D(timeOriginalData, time1Data, timeDataType, nTimes,
                    false);
            ...
            if (hasZeta) {
                ...
                NetCDFConverterUtilities.setData1D(levelOriginalData, zeta1Data, zetaDataType,
                        nZeta, false);
            ...
            for (Object object : ncFileIn.getVariables()) {
                ...
                if (var != null) {
                    ...
                    if (!Double.isNaN(fV)) {
                        outNoData = (float) METOCSActionsIOUtils.rescaleValue(fV, scale, offset);
                    ...
                    Array destArray = NetCDFConverterUtilities.getArray(originalVarData.getShape(),
                            DataType.DOUBLE);
                    for (int t = 0; t < nTimes; t++)
                        for (int z = 0; z < nZeta; z++)
                            for (int y = 0; y < nLat; y++)
                                for (int x = 0; x < nLon; x++) {
                                    int originalValue = originalVarData.getInt(originalVarData
                                            .getIndex().set(t, z, y, x));
                                    destArray
                                            .setDouble(
                                                    destArray.getIndex().set(t, z, y, x),
                                                    (float) ((originalValue != noData && originalValue != fillValue) ? METOCSActionsIOUtils
                                                            .rescaleValue(originalValue, scale,
                                                                    offset)
                                                            : outNoData));
                                }
