...
public class JGSFLoDeSSNCOMFileConfigurator extends BaseAction<FileSystemEvent> {
    ...
    private final static Logger LOGGER = LoggerFactory.getLogger(JGSFLoDeSSNCOMFileConfigurator.class.toString());
    ...
    public Queue<FileSystemEvent> execute(Queue<FileSystemEvent> events)
        ...
        try {
            ...
            final File ncomsDatasetDirectory = ("zip".equalsIgnoreCase(fileSuffix) || "tar"
                    .equalsIgnoreCase(fileSuffix)) ? Utilities.decompress("NCOM",
                    event.getSource(), tempFile) : Utilities.createTodayPrefixedDirectory("NCOM",
                    outDir);
            ...
            if (NCOMGridAreaFiles.length != 1) {
                if (LOGGER.isErrorEnabled())
                    LOGGER.error("Could not find NCOM Grid Area definition file.");
            ...
            if (NCOMVarFolders.length == 0) {
                if (LOGGER.isErrorEnabled())
                    LOGGER.error("Could not find NCOM Variables.");
            ...
            // ".nc"
            // ////
            final File outputFile = new File(outDir, "JGSFLoDeSS_NCOM-Forecast-T"
                    + new Date().getTime()
            ...
            int t1 = (timesFound.size() > 0 ? Integer.parseInt(timesFound.get(1).substring(
                    timesFound.get(1).lastIndexOf("_") + 1)) : t0);
                                ...
                                if (!varName.equalsIgnoreCase("X_Index")
                                    ...
                                    if (foundVariables.get(varName) == null) {
                                        ...
                                        for (MetocElementType m : metocDictionary.getMetoc()) {
                                            if ((varName.equalsIgnoreCase("U_Velocity") && m
                                                    .getName().equals("water velocity u-component"))
                                                    || (varName.equalsIgnoreCase("V_Velocity") && m
                                                            .getName().equals(
            ...
            final List<Dimension> outDimensions = METOCSActionsIOUtils
                    .createNetCDFCFGeodeticDimensions(ncFileOut, true, timesFound.size(), true,
                            depth.getLength(), METOCSActionsIOUtils.DOWN, true, yIndex.getLength(),
            ...
            for (String varName : foundVariables.keySet()) {
                ...
                if (variableBrief != null) {
                    ...
                    ncFileOut.addVariableAttribute(variableBrief, "units", foundVariableUoM
                            .get(varName));
                    ...
                    if (Double.isNaN(noData)) {
                        ...
                        Attribute missingValue = foundVariables.get(varName).findAttribute(
                                "missing_value");
                        if (missingValue != null) {
                            ...
                            if (variableBrief.equalsIgnoreCase("watvel-v")
                                    || (variableBrief.equalsIgnoreCase("watvel-u"))) {
                                noDataWater = noData;
                            }
                        }
                    }
            ...
            for (File NCOMVarFolder : NCOMVarFolders) {
                for (File NCOMVar : NCOMVarFolder.listFiles()) {
                    if (FilenameUtils.getExtension(NCOMVar.getName()).equalsIgnoreCase("nc")
                            || FilenameUtils.getExtension(NCOMVar.getName()).equalsIgnoreCase(
                                    "netcdf")) {
                        NetcdfFile ncVarFile = null;
                        try {
                            ncVarFile = NetcdfFile.open(NCOMVar.getAbsolutePath());
                            ...
                            for (Object obj : ncVarFile.getVariables()) {
                                ...
                                if (!varName.equalsIgnoreCase("X_Index")
                                    ...
                                    for (int z = 0; z < Depth.getLength(); z++) {
                                        ...
                                        if (!depthValuesFound.contains(depthOriginalData
                                                .getFloat(depthOriginalData.getIndex().set(z)))) {
                                            depthValuesFound.add(depthOriginalData
                                                    .getFloat(depthOriginalData.getIndex().set(z)));
                                        }
                                        ...
                                        METOCSActionsIOUtils.write2DData(userRaster, var,
                                                originalVarArray, false, false, new int[] { z,
                                                        yIndex.getLength(), xIndex.getLength() },
                                                false);

                                        // Resampling to a Regular Grid ...
                                        if (LOGGER.isTraceEnabled())
                                            LOGGER.trace("Resampling to a Regular Grid ...");
                                        userRaster = METOCSActionsIOUtils.warping(bbox,
                                                lonOriginalData, latOriginalData, xIndex
                                                        .getLength(), yIndex.getLength(), 2,
                                                userRaster, (float) noData, false);
                                        ...
                                        int tIndex = 0;
                                        String baseName = FilenameUtils.getBaseName(NCOMVar
                                                .getName());
                                        String timeInstant = baseName.substring(baseName.length()
                                                - "yyyyMMdd00_hhh".length());
                                        ...
                                        for (int y = 0; y < yIndex.getLength(); y++)
                                            for (int x = 0; x < xIndex.getLength(); x++) {
                                                ...
                                                outVarData
                                                        .setDouble(
                                                                outVarData.getIndex().set(tIndex,
                                                                        z, y, x),
                            ...
                            for (int y = 0; y < yIndex.getLength(); y++) {
                                for (int x = 0; x < xIndex.getLength(); x++) {
                                    ...
                                    double magnitudeValue = (uValue != noData && vValue != noData) ? Math
                                            .sqrt(Math.pow(uValue, 2) + Math.pow(vValue, 2))
                                            : noData;
