...
public class METOCSActionsIOUtils {
	...
	protected final static Logger LOGGER = Logger
			.getLogger(METOCSActionsIOUtils.class.toString());
	...
	public static Array write2DData(WritableRaster userRaster, Variable var,
			final Array originalVarData, final boolean findNewRange,
			final boolean updateFillValue, final int[] loopLengths,
			final boolean flipY) throws IOException, InvalidRangeException {
		return write2DData(userRaster, var, originalVarData, findNewRange, updateFillValue, loopLengths, flipY, null, false);
	}
	/**
	 * 
	 * @param userRaster
	...
	public static Array write2DData(WritableRaster userRaster, Variable var,
			final Array originalVarData, final boolean findNewRange,
			final boolean updateFillValue, final int[] loopLengths,
			final boolean flipY, final Array mask, final boolean maskOneIsValid) throws IOException, InvalidRangeException {
		...
		if (varDataType == DataType.FLOAT) {
			...
			for (int yPos = 0; yPos < latPositions; yPos++) {
				for (int xPos = 0; xPos < lonPositions; xPos++) {
					float sVal = originalVarData
							.getFloat(tPos >= 0 ? (zPos >= 0 ? varIndex.set(
									tPos, zPos, yPos, xPos) : varIndex.set(
									tPos, yPos, xPos)) : (zPos >= 0 ? varIndex
									.set(zPos, yPos, xPos) : varIndex.set(yPos,
									xPos)));
					if (mask != null) {
						int validByte = maskOneIsValid ? 1 : 0;
						if (mask.getByte(maskIndex.set(yPos,
										xPos)) != validByte) {
							sVal = fillValue;
						}
					}
			...
			for (int yPos = 0; yPos < latPositions; yPos++) {
				for (int xPos = 0; xPos < lonPositions; xPos++) {
					double sVal = originalVarData
							.getDouble(tPos >= 0 ? (zPos >= 0 ? varIndex.set(
									tPos, zPos, yPos, xPos) : varIndex.set(
									tPos, yPos, xPos)) : (zPos >= 0 ? varIndex
									.set(zPos, yPos, xPos) : varIndex.set(yPos,
									xPos)));
		...
		// //
			...
			for (int yPos = 0; yPos < latPositions; yPos++) {
				for (int xPos = 0; xPos < lonPositions; xPos++) {
					byte sVal = originalVarData
							.getByte(tPos >= 0 ? (zPos >= 0 ? varIndex.set(
									tPos, zPos, yPos, xPos) : varIndex.set(
									tPos, yPos, xPos)) : (zPos >= 0 ? varIndex
									.set(zPos, yPos, xPos) : varIndex.set(yPos,
									xPos)));
		...
		// //
			...
			for (int yPos = 0; yPos < latPositions; yPos++) {
				for (int xPos = 0; xPos < lonPositions; xPos++) {
					short sVal = originalVarData
							.getShort(tPos >= 0 ? (zPos >= 0 ? varIndex.set(
									tPos, zPos, yPos, xPos) : varIndex.set(
									tPos, yPos, xPos)) : (zPos >= 0 ? varIndex
									.set(zPos, yPos, xPos) : varIndex.set(yPos,
									xPos)));
		...
		// //
			...
			for (int yPos = 0; yPos < latPositions; yPos++) {
				for (int xPos = 0; xPos < lonPositions; xPos++) {
					int sVal = originalVarData
							.getInt(tPos >= 0 ? (zPos >= 0 ? varIndex.set(tPos,
									zPos, yPos, xPos) : varIndex.set(tPos,
									yPos, xPos)) : (zPos >= 0 ? varIndex.set(
									zPos, yPos, xPos) : varIndex
									.set(yPos, xPos)));
					if (mask != null) {
						int validByte = maskOneIsValid ? 1 : 0;
						if (mask.getByte(maskIndex.set(yPos,
	 ...
	 * @param flipY
	 */
	public static void write2DData(WritableRaster userRaster,
			final FlatFileGrid fileGrid, final boolean findNewRange,
			final boolean flipY) {
		double min = Double.MAX_VALUE;
		double max = Double.MIN_VALUE;
	 ...
	 * @return
	 */
	public static double[] computeExtrema(final Array latOriginalData,
			final Array lonOriginalData, final Dimension Y_Index,
			final Dimension X_Index) {
		...
		if (latOriginalData.getRank() == 1 && lonOriginalData.getRank() == 1) {
			for (int Y = 0; Y < Y_Index.getLength(); Y++) {
				double lat = latOriginalData.getDouble(latOriginalData
						.getIndex().set(Y));
			...
			for (int X = 0; X < X_Index.getLength(); X++) {
				double lon = lonOriginalData.getDouble(lonOriginalData
						.getIndex().set(X));

				extrema[0] = lon < extrema[0] ? lon : extrema[0];
				extrema[2] = lon > extrema[2] ? lon : extrema[2];
			}
		} else if (latOriginalData.getRank() == 2
				&& lonOriginalData.getRank() == 2) {
			for (int X = 0; X < X_Index.getLength(); X++)
				for (int Y = 0; Y < Y_Index.getLength(); Y++) {
					double lon = lonOriginalData.getDouble(lonOriginalData
							.getIndex().set(Y, X));
	 ...
	 * @param flipY
	 */
	public static WritableRaster warping(final double[] bbox,
			final Array lonData, final Array latData, final int imageWidth,
			final int imageHeight, final int polyDegree,
			final WritableRaster data, final float fillValue,
			final boolean flipY) {
		...
		for (int yi = 0; yi < imageHeight; yi += stepY) {
			for (int xi = 0; xi < imageWidth; xi += stepX) {
				...
				destCoords[offset] = (float) ((lonData.getFloat(lonData
						.getIndex().set(yi, xi)) - bbox[0]) / periodX);
				// Flipping y
				if (flipY) {
					destCoords[offset + 1] = (float) ((bbox[3] - latData
							.getFloat(latData.getIndex().set(yi, xi))) / periodY);
				} else {
					destCoords[offset + 1] = (float) ((latData.getFloat(latData
							.getIndex().set(yi, xi)) - bbox[1]) / periodY);
				}
		...
		WritableRaster target = RasterFactory.createWritableRaster(data
				.getSampleModel(), null);
			...
			for (int yi = 0; yi < imageHeight; yi++) {
				for (int xi = 0; xi < imageWidth; xi++) {
					...
					if (X >= 0 && Y >= 0 && X < imageWidth && Y < imageHeight) {
						target.setSample(xi, yi, bi, data.getSampleFloat(X, Y,
								bi));
					} else {
	 ...
	 * @param flipY
	 */
	public static WritableRaster warping(final FlatFileGrid fileGrid,
		...
		for (int yi = 0; yi < imageHeight; yi += stepY) {
			for (int xi = 0; xi < imageWidth; xi += stepX) {
				...
				// Flipping y
				if (flipY) {
					destCoords[offset + 1] = (float) ((bbox[3] - latData[yi
							* imageHeight + xi]) / periodY);
				} else {
					destCoords[offset + 1] = (float) ((latData[yi * imageHeight
							+ xi] - bbox[1]) / periodY);
				}
	 ...
	 * 
	 * time { time:long_name = "time" time:units =
	 * "seconds since 1980-1-1 0:0:0" }
	 * 
	...
	public static List<Dimension> createNetCDFCFGeodeticDimensions(
			NetcdfFileWriteable ncFileOut, final boolean hasTimeDim,
			final int tDimLength, final boolean hasZetaDim,
			final int zDimLength, final String zOrder, final boolean hasLatDim,
			final int latDimLength, final boolean hasLonDim,
			final int lonDimLength, final DataType dataType) {
		final List<Dimension> dimensions = new ArrayList<Dimension>();

		if (hasTimeDim) {
			Dimension timeDim = ncFileOut.addDimension(TIME_DIM, tDimLength);

			ncFileOut.addVariable(TIME_DIM, dataType,
					new Dimension[] { timeDim });
			ncFileOut.addVariableAttribute(TIME_DIM, LONG_NAME, TIME_DIM);
			ncFileOut.addVariableAttribute(TIME_DIM, UNITS,
					"seconds since 1980-1-1 0:0:0");
		...
		if (hasZetaDim) {
			Dimension zDim = ncFileOut.addDimension(
					zOrder.equals(DOWN) ? DEPTH_DIM : HEIGHT_DIM, zDimLength);

			ncFileOut.addVariable(zOrder.equals(DOWN) ? DEPTH_DIM : HEIGHT_DIM,
					DataType.FLOAT, new Dimension[] { zDim });
			ncFileOut.addVariableAttribute(zOrder.equals(DOWN) ? DEPTH_DIM
					: HEIGHT_DIM, LONG_NAME, NetCDFUtilities.DEPTH);
			ncFileOut.addVariableAttribute(zOrder.equals(DOWN) ? DEPTH_DIM
					: HEIGHT_DIM, UNITS, "m");
			ncFileOut.addVariableAttribute(zOrder.equals(DOWN) ? DEPTH_DIM
					: HEIGHT_DIM, POSITIVE, zOrder);
		...
		if (hasLatDim) {
			...
			ncFileOut.addVariable(LAT_DIM, DataType.FLOAT,
					new Dimension[] { latDim });
			ncFileOut.addVariableAttribute(LAT_DIM, LONG_NAME,
		...
		if (hasLonDim) {
			...
			ncFileOut.addVariable(LON_DIM, DataType.FLOAT,
					new Dimension[] { lonDim });
			ncFileOut.addVariableAttribute(LON_DIM, LONG_NAME,
	...
	public static List<Dimension> createNetCDFCFGeodeticDimensions(
		...
		return createNetCDFCFGeodeticDimensions(ncFileOut, hasTimeDim,
				tDimLength, hasZetaDim, zDimLength, zOrder, hasLatDim,
				latDimLength, hasLonDim, lonDimLength, DataType.FLOAT);
	...
	public static boolean sendHarvestRequest(final String registryURL,
		...
		HttpURLConnection con = (HttpURLConnection) registryWS_URL
				.openConnection();
	...
	public static double rescaleValue(int value, Number scale, Number offset) {
		if (scale == null || offset == null)
			throw new IllegalArgumentException(
					"Scale and Offset shouldn't be NULL");
		return (value * (scale instanceof Float ? scale.floatValue() : scale
				.doubleValue()))
				+ (offset instanceof Float ? offset.floatValue() : offset
						.doubleValue());
	}
