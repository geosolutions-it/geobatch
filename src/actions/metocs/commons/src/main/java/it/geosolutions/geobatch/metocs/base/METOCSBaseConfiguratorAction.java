 ...
 * 
 */
public abstract class METOCSBaseConfiguratorAction extends
	...
	public Queue<FileSystemMonitorEvent> execute(
		...
		try {
			...
			final File tempFile = File.createTempFile(inputFileName, ".tmp",
					outDir);
			final File metocsDatasetDirectory = unzipMetocArchive(event,
					fileSuffix, outDir, tempFile);
	 ...
	 * @param TAU
	 */
	protected void settingNCGlobalAttributes(double noData,
			final Date timeOriginDate, int TAU) {
	 ...
	 * 
	 */
	protected abstract void writeDownNetCDF(File outDir, String inputFileName)
	...
	protected abstract File unzipMetocArchive(FileSystemMonitorEvent event,
			final String fileSuffix, final File outDir, final File tempFile)
			throws IOException;
}
