...
public class AnnotationProcessingTest {
	...
	@Test
	public void testXstreamConfigurationAliased() throws Exception{

		Boolean configurationAliasRegistered = false;
		Class configurationClass = XstreamConfiguration.class;
		Iterator<Entry<String, Class<?>>> it = aliasRegistry.iterator();
		while (it.hasNext()){
			Entry<String, Class<?>> alias = it.next();	

			Action action = (Action)XstreamAction.class.getAnnotations()[0];
	...
	@Test
	public void testXstreamActionServiceGeneration() throws Exception{
		...
		for(GenericActionService action : actionList){
			if(XstreamConfiguration.class.getSimpleName().equals(action.getId())){
