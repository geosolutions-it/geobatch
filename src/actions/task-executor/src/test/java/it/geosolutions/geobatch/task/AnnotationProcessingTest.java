...
public class AnnotationProcessingTest {
	...
	@Test
	public void testTaskExecutorConfigurationAliased() throws Exception{

		Boolean configurationAliasRegistered = false;
		Class configurationClass = TaskExecutorConfiguration.class;
		Iterator<Entry<String, Class<?>>> it = aliasRegistry.iterator();
		while (it.hasNext()){
			Entry<String, Class<?>> alias = it.next();	

			Action action = (Action)TaskExecutor.class.getAnnotations()[0];
	...
	@Test
	public void testTaskExecutorServiceGeneration() throws Exception{
		...
		for(GenericActionService action : actionList){
			if(TaskExecutorConfiguration.class.getSimpleName().equals(action.getId())){
