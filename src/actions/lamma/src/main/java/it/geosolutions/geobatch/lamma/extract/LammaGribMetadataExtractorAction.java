...
public class LammaGribMetadataExtractorAction extends LammaBaseAction {
    ...
    protected final static Logger LOGGER = Logger.getLogger(LammaGribMetadataExtractorAction.class.toString());
     ...
     * @param configuration
     */
    public LammaGribMetadataExtractorAction(LammaGribMetadataExtractorConfiguration configuration) throws IOException {
        super(configuration);
    ...
    public Queue<FileSystemMonitorEvent> execute(Queue<FileSystemMonitorEvent> events)
        ...
        try {
            ...
            final File srcGribFile = new File(getScriptArguments(inputFile.getAbsolutePath(), "srcfile"));
            final File gribInventoryFile = new File(getScriptArguments(inputFile.getAbsolutePath(), "destination"));

            // Logging to ESB ...
            logMessage.setMessage("Initliazing Grib inventory for file: " + srcGribFile.getName());
            ...
            if (inventory.isValid() && inventory.isInitialized()) {
                // Logging to ESB ...
                logMessage.setMessage("Grib inventory successfully initialized for file: " + srcGribFile.getName());
                logMessage.setMessageTime(new Date());
    			logToESB(logMessage);
            	...
            	for (Long rId : inventory.getRecords().keySet()) {
            		WgribRecordDescriptor record = inventory.getRecords().get(rId);
            		
            		final String dirName = SUPPORTED_PARAMS.valueOf(record.getParamId())
            				.getParamDescription().replaceAll(" ", "_").replaceAll("\\.", "")
            				.trim();
					final File paramDir = createParamBaseDir(configuration.getBaseOutputDir(), dirName);

            		final File metadataFile = extractGribMetadata(srcGribFile, paramDir, run, record);
//                    // Logging to ESB ...
//                    logMessage.setMessage("Created Metadata file: " + metadataFile.getName());
//                    logMessage.setMessageTime(new Date());
//        			logToESB(logMessage);
					events.add(new FileSystemMonitorEvent(metadataFile, FileSystemMonitorNotifications.FILE_ADDED));
            	}
            } else {
    			// Logging to ESB ...
                logMessage.setMessage("[ERROR] " + "Grib Inventory could not be initialized.");
                logMessage.setMessageTime(new Date());
    			logToESB(logMessage);

            	throw new IllegalStateException("Grib Inventory could not be initialized.");
            }
            
            // Logging to ESB ...
            logMessage.setMessage("Successfully created parameters metadata for file: " + srcGribFile.getName());
            logMessage.setMessageTime(new Date());
			logToESB(logMessage);
     ...
     * @return
     */
    private static File extractGribMetadata(final File srcGribFile, File paramDir, final String run, final WgribRecordDescriptor record) {
        ...
        calendar.add(GregorianCalendar.HOUR, Integer.parseInt(record.getForecastTime()));
        
    	StringBuilder metadataFileName = new StringBuilder("lamma")
    		.append("_").append("R").append(record.getrId())
    		.append("_").append(record.getParamId())
    		.append("_").append(record.getLevel()) // Z min
    		.append("_").append(record.getLevel()) // Z max
    		.append("_").append(runtimedf.format(record.getBaseTime()))
    		.append("_").append(sdf.format(calendar.getTime()))
    		.append("_").append(run)
    		.append("_").append("9.999e20"); // NoDATA
    	 ...
    	 */
    	final File gribMetadata = new File(paramDir, metadataFileName.toString()+".info");
    	try {
    	 ...
    	 */
    	final File taskExecutorInput = new File(paramDir, metadataFileName.toString()+".xml");
    	try {
    		...
    		out.println("  <text/>");
        	final File wgribRecordDump = new File(paramDir, metadataFileName.toString()+".dat");
    		out.println("  <o>"+wgribRecordDump.getAbsolutePath()+"</o>");
    		out.println("  <V/>");
    		out.println("  <srcfile>"+srcGribFile.getAbsolutePath()+"</srcfile>");
     ...
     * @return 
     */
    private static File createParamBaseDir(final String baseDir, final String dirName) {
