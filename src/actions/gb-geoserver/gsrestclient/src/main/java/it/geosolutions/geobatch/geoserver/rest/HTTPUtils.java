...
public class HTTPUtils {
	...
	public static String get(String url, String username, String pw)
		...
		try {
			...
			if (status == HttpStatus.SC_OK) {
				...
				LOGGER.info("(" + status + ") "
						+ HttpStatus.getStatusText(status) + " -- " + url);
			}
	...
	public static boolean put(String url, File file, String contentType,
			String username, String pw) {
		return put(url, new FileRequestEntity(file, contentType), username, pw);
	}

	public static boolean put(String url, String content, String contentType,
			String username, String pw) {
		try {
			return put(url,
					new StringRequestEntity(content, contentType, null),
					username, pw);
	...
	public static boolean putXml(String url, String content, String username,
			String pw) {
		return put(url, content, "text/xml", username, pw);
	}

	public static boolean put(String url, RequestEntity requestEntity,
			String username, String pw) {
		return send(new PutMethod(url), url, requestEntity, username, pw);
	}

	public static boolean post(String url, File file, String contentType,
			String username, String pw) {
		return post(url, new FileRequestEntity(file, contentType), username, pw);
	}

	public static boolean post(String url, String content, String contentType,
			String username, String pw) {
		try {
			return post(url,
					new StringRequestEntity(content, contentType, null),
	...
	public static boolean postXml(String url, String content, String username,
			String pw) {
		return post(url, content, "text/xml", username, pw);
	}

	public static boolean post(String url, RequestEntity requestEntity,
			String username, String pw) {
		return send(new PostMethod(url), url, requestEntity, username, pw);
	}

	private static boolean send(final EntityEnclosingMethod httpMethod,
			String url, RequestEntity requestEntity, String username, String pw) {
		boolean res = false;
		...
		try {
			...
			case HttpURLConnection.HTTP_ACCEPTED:
				String response = IOUtils.toString(httpMethod
			...
			default:
				LOGGER
						.warn("Bad response: code["
								...
								+ "]"
								+ " method["
								+ httpMethod.getClass().getSimpleName()
								+ "]: "
								+ IOUtils.toString(httpMethod
										.getResponseBodyAsStream()));
				return false;
			}
		} catch (ConnectException e) {
	...
	public static boolean delete(String url, final String user, final String pw) {
		...
		try {
			...
			if (status == HttpStatus.SC_OK) {
				...
				if (LOGGER.isDebugEnabled())
					LOGGER.debug("(" + status + ") "
							+ httpMethod.getStatusText() + " -- " + url);
				return true;
			} else {
				LOGGER.info("(" + status + ") " + httpMethod.getStatusText()
						+ " -- " + url);
	...
	public static boolean httpPing(String url, String username, String pw) {
		...
		try {
			...
			int status = client.executeMethod(httpMethod);
			if (status != HttpStatus.SC_OK) {
				LOGGER.warn("PING failed at '" + url + "': (" + status + ") "
						+ httpMethod.getStatusText());
				return false;
			} else {
				return true;
	 ...
	 * 
	 * @param url
	 *            The URL of the REST resource to query about.
	 * @param username
	 * @param pw
	 * @return true on 200, false on 404.
	 * @throws RuntimeException
	 *             on unhandled status or exceptions.
	 */
	public static boolean exists(String url, String username, String pw) {
		...
		try {
			...
			case HttpStatus.SC_NOT_FOUND:
				return false;
			default:
				throw new RuntimeException("Unhandled response status at '"
						+ url + "': (" + status + ") "
						+ httpMethod.getStatusText());
			}
		} catch (ConnectException e) {
	...
	private static void setAuth(HttpClient client, String url, String username,
			String pw) throws MalformedURLException {
		URL u = new URL(url);
		if (username != null && pw != null) {
			Credentials defaultcreds = new UsernamePasswordCredentials(
					username, pw);
			client.getState().setCredentials(
					new AuthScope(u.getHost(), u.getPort()), defaultcreds);
