...
/**
 * Connect to a GeoServer instance and read its data. <BR>
 * Info are returned as Strings or, for complex data, as JDOM elements. You may
 * use the {@link parser.GSRestLayerParser},
 * {@link parser.GSRestDatastoreParser}, etc to retrieve info from these xml
 * trees.
 * 
 * <P>
 * <B>TODO: username and pw are now mandatory also for getting data from GS.</B>
 * 
 * @author ETj <etj at geo-solutions.it>
 */
public class GeoServerRESTReader {
	private final static Logger LOGGER = Logger
			.getLogger(GeoServerRESTReader.class.getName());
	...
	public Element getDatastore(String workspace, String dsName) {
		String url = "/rest/workspaces/" + workspace + "/datastores" + dsName
				+ ".xml";
		if (LOGGER.isLoggable(Level.FINE))
	...
	public List<String> getNamespaceNames() {
		...
		if (namespaces == null)
			return null;
		for (Element namespace : (List<Element>) namespaces
	...
	public List<String> getWorkspaceNames() {
		...
		if (workspaces == null)
			return null;
		for (Element workspace : (List<Element>) workspaces
