...
//				final String configId = configuration.getName();
				...
				if (input.exists() && input.isFile() && FilenameUtils.getExtension(input.getName()).equalsIgnoreCase("xml")) {
				...
				if (!indexer.exists()) {
					try {
						...
						out.println("PropertyCollectors=" + 
								...
								(configuration.getRuntimeRegex() != null ? (configuration.getTimeRegex() != null || configuration.getElevationRegex() != null ? "," : "") + "RuntimeFileNameExtractorSPI[runtimeregex](runtime)" : "")
						);
					} catch (IOException e){
				...
				if (!runtimeregex.exists()) {
					if (configuration.getRuntimeRegex() != null) {
						try {
							...
							LOGGER.log(Level.SEVERE, "Error occurred while writing runtimeregex.properties file!", e);
						} finally {
							if (out != null) {
								out.flush();
								out.close();
						...
						if(Utils.checkFileReadable(datastore)){
							...
							try {
								...
								if(spi instanceof PostgisNGJNDIDataStoreFactory||spi instanceof PostgisNGDataStoreFactory)
								{
									...
									SimpleFeatureSource featureSource = tileIndexStore.getFeatureSource(typeName);
									...
									try{
										...
										List<String> newGranules = new ArrayList<String>(fileNameList);
										while (it.hasNext()) {
											// get the feature
											final SimpleFeature sf = it.next();
											String location = (String) sf.getAttribute("location");
											
											int index = Collections.binarySearch(fileNameList, location);
											if (index >= 0) {
										...
										if (newGranules.size() > 0) {
											...
											Geometry the_geom = wktReader.read(
													"POLYGON(("+bounds.getMinX()+" "+bounds.getMinY()+","+bounds.getMinX()+" "+bounds.getMaxY()+","+bounds.getMaxX()+" "+bounds.getMaxY()+","+bounds.getMaxX()+" "+bounds.getMinY()+","+bounds.getMinX()+" "+bounds.getMinY()+"))");
											Integer SRID = CRS.lookupEpsgCode(bounds.getCoordinateReferenceSystem(), true);
											the_geom.setSRID(SRID);
											
											Properties ftProps = Utils.loadPropertiesFromURL(DataUtilities.fileToURL(indexer));
											...
											fw = tileIndexStore.getFeatureWriterAppend(typeName, Transaction.AUTO_COMMIT);
											...
											for (String granuleLocation : newGranules) {
												...
												feature.setAttribute(geometryPropertyName, the_geom);
												feature.setAttribute("location", granuleLocation);
												
												if (ftProps.getProperty("TimeAttribute") != null && timePattern != null) {
													final Matcher matcher = timePattern.matcher(granuleLocation);
													if (matcher.find()) {
														...
														if (dates != null && dates.size() > 0) {
															...
															feature.setAttribute(ftProps.getProperty("TimeAttribute"), cal.getTime());
														}
													}
												}

												if (ftProps.getProperty("ElevationAttribute") != null) {
													final Matcher matcher = elevPattern.matcher(granuleLocation);
													if (matcher.find()) {
														feature.setAttribute(ftProps.getProperty("ElevationAttribute"), Double.valueOf(matcher.group()));
													}
												}
												
												if (ftProps.getProperty("RuntimeAttribute") != null) {
													final Matcher matcher = runtimePattern.matcher(granuleLocation);
													if (matcher.find()) {
														feature.setAttribute(ftProps.getProperty("RuntimeAttribute"), Integer.valueOf(matcher.group()));
													}
												}
										...
										File layerDescriptor = new File(inputDir, mosaicDescriptor.coverageStoreId + ".layer");
	...
	private static File parseImageMosaicDirectory(File xmlFile) {
		...
		if (xmlFile!=null){
			try {
				...
				LOGGER.warning(new StringBuilder("Problems occurred while reading: ").append(xmlFile).append("due to ").append(e.getLocalizedMessage()).toString());
			}
		}
	 ...
	 */
	private boolean checkIfImageMosaicLayerExists(String coverageStoreId) throws ParserConfigurationException, IOException, TransformerException {
	 ...
	 * @throws TransformerException
	 */
	private void createNewImageMosaicLayer(File inputDir, ImageMosaicGranulesDescriptor mosaicDescriptor, Collection<FileSystemMonitorEvent> layers) throws ParserConfigurationException, IOException,
		...
		final String[] layerResponse = GeoServerRESTHelper.sendCoverage(
				...
				getConfiguration().getGeoserverUID(), 
				getConfiguration().getGeoserverPWD(),
				mosaicDescriptor.coverageStoreId, 
				mosaicDescriptor.coverageStoreId,
				queryParams, 
				"", 
				"EXTERNAL",
		...
		if (layerResponse != null && layerResponse.length > 2) {
			...
			String noData = (mosaicDescriptor.firstCvNameParts.length >= 9 ? mosaicDescriptor.firstCvNameParts[8] : mosaicDescriptor.firstCvNameParts.length >= 8 ? mosaicDescriptor.firstCvNameParts[7] : "-1.0");
	...
	private ImageMosaicGranulesDescriptor createImageMosaicDescriptor(File inputDir) {
		...
		if (fileNames != null && fileNames.length > 0) {
			...
			if (firstCvNameParts != null && firstCvNameParts.length > 3) {
				...
				mosaicDescriptor = new ImageMosaicGranulesDescriptor(coverageStoreId, metocFields, firstCvNameParts, lastCvNameParts);
			}
		}
	...
	final class ImageMosaicGranulesDescriptor {
		 ...
		 */
		public ImageMosaicGranulesDescriptor(String coverageStoreId,
				String metocFields, String[] firstCvNameParts,
				String[] lastCvNameParts) {
