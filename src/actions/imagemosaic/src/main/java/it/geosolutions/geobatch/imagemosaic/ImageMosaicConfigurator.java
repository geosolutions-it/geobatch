...
public class ImageMosaicConfigurator extends
			...
			while (events.size() > 0) {
				...
				if (!timeregex.exists()) {
					if (configuration.getTimeRegex() != null) {
						try {
							...
							out
									.println("regex="
											+ configuration.getTimeRegex());
						} catch (IOException e) {
							LOGGER
									.log(
											Level.SEVERE,
											"Error occurred while writing timeregex.properties file!",
											e);
						} finally {
							if (out != null) {
								out.flush();
				...
				if (configuration.getDatastorePropertiesPath() != null) {
					...
					if (dsFile != null && dsFile.exists()
							&& !dsFile.isDirectory()) {
						LOGGER.info("DataStore file found: "
				...
				if (!exists) {
					...
					if (mosaicDescriptor != null)
						createNewImageMosaicLayer(inputDir, mosaicDescriptor,
								layers);
				} else {
					/**
					...
					if (datastore.exists()) {
						...
						if (Utils.checkFileReadable(datastore)) {
							...
							// SPI
							final String SPIClass = properties
									.getProperty("SPI");
							try {
								...
								if (spi instanceof PostgisNGJNDIDataStoreFactory
									...
									DataStore tileIndexStore = spi
											.createDataStore(params);
									final String[] typeNames = tileIndexStore
											.getTypeNames();
									...
									final FeatureType schema = featureSource
											.getSchema();
									...
									try {
										...
										String[] fileNames = inputDir
												.list(new SuffixFileFilter(
														new String[] { ".tif",
																".tiff" },
														IOCase.INSENSITIVE));
										...
										while (it.hasNext()) {
											...
											String location = (String) sf
													.getAttribute("location");

											int index = Collections
													.binarySearch(fileNameList,
															location);
											if (index >= 0) {
										...
										try {
											if (newGranules.size() > 0) {
												...
												for (String granuleLocation : newGranules) {
													...
													final SimpleFeature feature = fw
															.next();
													...
													if (ftProps
															.getProperty("TimeAttribute") != null
															&& timePattern != null) {
													...
													if (ftProps
															.getProperty("ElevationAttribute") != null) {
													...
													if (ftProps
															.getProperty("RuntimeAttribute") != null) {
										...
										File layerDescriptor = new File(
												inputDir,
												mosaicDescriptor.coverageStoreId
														+ ".layer");
										if (layerDescriptor.exists()
												&& layerDescriptor.isFile())
											layers
						...
						if (layerDescriptor.exists()
								&& layerDescriptor.isFile())
							layers.add(new FileSystemMonitorEvent(
									layerDescriptor,
									FileSystemMonitorNotifications.FILE_ADDED));
					}
				}
	...
	private static File parseImageMosaicDirectory(File xmlFile) {
		...
		if (xmlFile != null) {
			try {
				final FileImageInputStream fis = new FileImageInputStream(
						xmlFile);
					...
					if (location.startsWith(startTag)) {
						if (location.endsWith(endTag)) {
							dataDir = location.substring(
									location.indexOf(startTag)
											+ startTag.length(),
									location.length() - (endTag.length()))
									.trim();
						} else {
							String next = fis.readLine();
							if (next != null) {
								if (next.endsWith(endTag)) {
									dataDir = next.substring(0,
											next.length() - (endTag.length()))
											.trim();
								} else {
									...
									if (nextLine != null) {
										...
										LOGGER
												.warning("Unable to find missions");
										return null;
									}
								}
							} else {
				...
				LOGGER.warning(new StringBuilder(
						"Problems occurred while reading: ").append(xmlFile)
						.append("due to ").append(e.getLocalizedMessage())
						.toString());
			}
		}
	...
	private boolean checkIfImageMosaicLayerExists(String coverageStoreId)
		...
		return GeoServerRESTHelper.checkLayerExistence(getConfiguration()
				.getGeoserverURL(), getConfiguration().getGeoserverUID(),
				getConfiguration().getGeoserverPWD(), coverageStoreId);
	}
	...
	private void createNewImageMosaicLayer(File inputDir,
			...
			Collection<FileSystemMonitorEvent> layers)
			throws ParserConfigurationException, IOException,
			TransformerException {
		FileWriter outFile;
		...
		final String[] layerResponse = GeoServerRESTHelper.sendCoverage(
				inputDir, inputDir, getConfiguration().getGeoserverURL(),
				getConfiguration().getGeoserverUID(), getConfiguration()
						.getGeoserverPWD(), mosaicDescriptor.coverageStoreId,
				mosaicDescriptor.coverageStoreId, queryParams, "", "EXTERNAL",
				"imagemosaic", GEOSERVER_VERSION, getConfiguration()
						.getStyles(), getConfiguration().getDefaultStyle());
		...
		if (layerResponse != null && layerResponse.length > 2) {
			...
			queryParams.clear();
			queryParams.put("MaxAllowedTiles", Integer
			...
			configureMosaic(queryParams, getConfiguration().getGeoserverURL(),
					getConfiguration().getGeoserverUID(), getConfiguration()
							.getGeoserverPWD(), workspace, coverageStore,
					coverageName);
	 ...
	 * @return
	 */
	private ImageMosaicGranulesDescriptor createImageMosaicDescriptor(
			File inputDir) {
		ImageMosaicGranulesDescriptor mosaicDescriptor = null;

		String[] fileNames = inputDir.list(new SuffixFileFilter(new String[] {
				".tif", ".tiff" }, IOCase.INSENSITIVE));
		List<String> fileNameList = Arrays.asList(fileNames);
		Collections.sort(fileNameList);
		fileNames = fileNameList.toArray(new String[1]);

		if (fileNames != null && fileNames.length > 0) {
			String[] firstCvNameParts = FilenameUtils.getBaseName(fileNames[0])
					.split("_");
			String[] lastCvNameParts = FilenameUtils.getBaseName(
					fileNames[fileNames.length - 1]).split("_");
			...
			if (firstCvNameParts != null && firstCvNameParts.length > 3) {
				...
				String metocFields = firstCvNameParts.length == 9
						...
						.append(firstCvNameParts[0]).append("_").append(
								firstCvNameParts[1]).append("_").append(
								firstCvNameParts[2]).append("_").append(
				...
				mosaicDescriptor = new ImageMosaicGranulesDescriptor(
						coverageStoreId, metocFields, firstCvNameParts,
						lastCvNameParts);
			}
		}
	...
	public static void configureMosaic(final Map<String, String> queryParams,
		...
		if (queryParams.containsKey("OutputTransparentColor")) {
			// Configuring wmsPath
			final String outputTransparentColor = queryParams
					.get("OutputTransparentColor");
			configElements
					.put("OutputTransparentColor", outputTransparentColor);
		} else {
		...
		if (queryParams.containsKey("InputTransparentColor")) {
			// Configuring wmsPath
			final String inputTransparentColor = queryParams
					.get("InputTransparentColor");
		...
		if (queryParams.containsKey("AllowMultithreading")) {
			// Configuring wmsPath
			final String allowMultithreading = queryParams
		...
		if (queryParams.containsKey("SUGGESTED_TILE_SIZE")) {
			// Configuring wmsPath
			final String suggestedTileSize = queryParams
					.get("SUGGESTED_TILE_SIZE");
		...
		if (!configElements.isEmpty()) {
			GeoServerRESTHelper.sendCoverageConfiguration(configElements,
					geoserverBaseURL, geoserverUID, geoserverPWD, workspace,
