...
public class AnnotationProcessingTest {
	...
	@Test
	public void testImageMosaicConfigurationAliased() throws Exception{

		Boolean configurationAliasRegistered = false;
		Class configurationClass = ImageMosaicConfiguration.class;
		Iterator<Entry<String, Class<?>>> it = aliasRegistry.iterator();
		while (it.hasNext()){
			Entry<String, Class<?>> alias = it.next();	

			Action action = (Action)ImageMosaicAction.class.getAnnotations()[0];
	...
	@Test
	public void testImageMosaicActionServiceGeneration() throws Exception{
		...
		for(GenericActionService action : actionList){
			if(ImageMosaicConfiguration.class.getSimpleName().equals(action.getId())){
