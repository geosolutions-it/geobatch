...
public class AnnotationProcessingTest {
	...
	@Test
	public void testFreeMarkerConfigurationAliased() throws Exception{
		Boolean configurationAliasRegistered = false;
		Class configurationClass = FreeMarkerConfiguration.class;
		Iterator<Entry<String, Class<?>>> it = aliasRegistry.iterator();
		while (it.hasNext()){
			Entry<String, Class<?>> alias = it.next();	
			
			Action action = (Action)FreeMarkerAction.class.getAnnotations()[0];
	...
	@Test
	public void testFreeMarkerActionServiceGeneration() throws Exception{
		...
		for(GenericActionService action : actionList){
			if(FreeMarkerConfiguration.class.getSimpleName().equals(action.getId())){
