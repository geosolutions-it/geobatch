...
public class ShipProcessor extends PackageProcessor<ShipDetectionPackage, ShipDetection, VesselRO>{
    ...
    @Override
    protected ShipDetection createPackageElement(ShipDetectionPackage pkg, PackedElement packed, File dir) throws JDOMException, IOException {
    ...
    public static VesselRO ship2vessel(ShipDetection ship) {
        VesselRO ro = new VesselRO();
        ro.setId(ship.getId());
        ro.setTimeStamp(ship.getTimestamp());
        ro.setEnvelope(
                ship.getX0()-0.00001d, ship.getX1()-0.00001d,
                ship.getX0()+0.00001d, ship.getX1()+0.00001d);
//        ro.setGmlURI("http://todo"); // TODO
        return ro;
    }
}
