...
import it.geosolutions.geobatch.actions.ds2ds.geoserver.DSGeoServerAction;
...
public class AnnotationProcessingTest {
	...
	@Test
	public void testDs2dsConfigurationAliased() throws Exception{
		Boolean configurationAliasRegistered = false;
		Class configurationClass = Ds2dsConfiguration.class;
		Iterator<Entry<String, Class<?>>> it = aliasRegistry.iterator();
		while (it.hasNext()){
			Entry<String, Class<?>> alias = it.next();	
			
			Action action = (Action)Ds2dsAction.class.getAnnotations()[0];
	...
	@Test
	public void testDs2dsActionServiceGeneration() throws Exception{
		...
		for(GenericActionService action : actionList){
			if(Ds2dsConfiguration.class.getSimpleName().equals(action.getId())){
	...
	@Test
	public void testDSGeoServerConfigurationAliased() throws Exception{
		Boolean configurationAliasRegistered = false;
		Class configurationClass = DSGeoServerConfiguration.class;
		Iterator<Entry<String, Class<?>>> it = aliasRegistry.iterator();
		while (it.hasNext()){
			Entry<String, Class<?>> alias = it.next();		
			
			Action action = (Action)DSGeoServerAction.class.getAnnotations()[0];
			String configurationAlias = action.configurationAlias();
			if(configurationAlias == null || configurationAlias.isEmpty()){
				configurationAlias = configurationClass.getSimpleName();
			}
	...
	@Test
	public void testDSGeoServerActionServiceGeneration() throws Exception{
		Boolean actionServiceRegistered = false;
		List<GenericActionService> actionList = ActionServicePostProcessor.getActionList();
		for(GenericActionService action : actionList){
			if(DSGeoServerConfiguration.class.getSimpleName().equals(action.getId())){
				actionServiceRegistered = true;
				break;
			}
		}
