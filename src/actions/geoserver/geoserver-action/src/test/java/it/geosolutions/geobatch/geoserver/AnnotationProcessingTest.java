...
import it.geosolutions.geobatch.geoserver.reload.GeoServerReload;
import it.geosolutions.geobatch.geoserver.reload.GeoServerReloadConfiguration;
import it.geosolutions.geobatch.geoserver.style.GeoServerStyleAction;
...
public class AnnotationProcessingTest {
	...
	@Test
	public void testGeoServerReloadActionConfigurationAliased() throws Exception{

		Boolean configurationAliasRegistered = false;
		Class configurationClass = GeoServerReloadConfiguration.class;
		Iterator<Entry<String, Class<?>>> it = aliasRegistry.iterator();
		while (it.hasNext()){
			Entry<String, Class<?>> alias = it.next();	

			Action action = (Action)GeoServerReload.class.getAnnotations()[0];
	...
	@Test
	public void testGeoServerReloadActionServiceGeneration() throws Exception{
		...
		for(GenericActionService action : actionList){
			if(GeoServerReloadConfiguration.class.getSimpleName().equals(action.getId())){
	...
	@Test
	public void testGeoServerStyleActionConfigurationAliased() throws Exception{

		Boolean configurationAliasRegistered = false;
		Class configurationClass = GeoServerStyleConfiguration.class;
		Iterator<Entry<String, Class<?>>> it = aliasRegistry.iterator();
		while (it.hasNext()){
			Entry<String, Class<?>> alias = it.next();	

			Action action = (Action) GeoServerStyleAction.class.getAnnotations()[0];
			String configurationAlias = action.configurationAlias();
			if(configurationAlias == null || configurationAlias.isEmpty()){
				configurationAlias = configurationClass.getSimpleName();
			}
	...
	@Test
	public void testGeoServerStyleActionServiceGeneration() throws Exception{

		Boolean actionServiceRegistered = false;
		List<GenericActionService> actionList = ActionServicePostProcessor.getActionList();
		for(GenericActionService action : actionList){
			if( GeoServerStyleConfiguration.class.getSimpleName().equals(action.getId())){
				actionServiceRegistered = true;
				break;
			}
		}
