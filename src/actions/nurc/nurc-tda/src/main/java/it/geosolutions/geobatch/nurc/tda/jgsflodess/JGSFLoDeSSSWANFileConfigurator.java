...
public class JGSFLoDeSSSWANFileConfigurator extends MetocConfigurationAction {
	...
	public Queue<FileSystemMonitorEvent> execute(
		...
		try {
			...
			Array time1Data = NetCDFConverterUtilities.getArray(nTimes,
					timeDataType);
			int TAU = 0;
			;
			for (int t = 0; t < nTimes; t++) {
				...
				if (t == 0 && nTimes > 1) {
					TAU = (int) Math
							.ceil((timeOriginalData.getLong(timeOriginalData
									.getIndex().set(t + 1)) - timeValue) / 3600.0);
				}
				// adding time offset
				timeValue = startTime + (timeValue * 3600000);
				// converting back to seconds and storing to data
				time1Data
						.setLong(time1Data.getIndex().set(t), timeValue / 1000);
			}
			...
			final List<Dimension> outDimensions = NURCActionsIOUtils
					.createNetCDFCFGeodeticDimensions(ncFileOut, true, timeDim
							.getLength(), hasZeta, nZeta,
							NURCActionsIOUtils.UP, true, latDim.getLength(),
			...
			for (Object obj : ncFileIn.getVariables()) {
				...
				if (var != null) {
					...
					if (varName.equalsIgnoreCase(NetCDFUtilities.LATITUDE)
							|| varName
									.equalsIgnoreCase(NetCDFUtilities.LONGITUDE)
					...
					if (foundVariables.get(varName) == null) {
						...
						for (MetocElementType m : metocDictionary.getMetoc()) {
							if ((varName.equalsIgnoreCase("sig_wav_ht") && m
									.getName()
									.equals("significant wave height"))
									|| (varName
											.equalsIgnoreCase("peak_wav_per") && m
											.getName().equals(
													"peak wave period"))
									|| (varName
											.equalsIgnoreCase("mean_wav_dir") && m
											.getName().equals(
													"mean wave direction"))) {
								longName = m.getName();
			...
			for (String varName : foundVariables.keySet()) {
				...
				if (variableBrief != null) {
					...
					if (!Double.isNaN(fillValue)) {
						outNoData = (float) NURCActionsIOUtils.rescaleValue(
								fillValue, scale, offset);
			...
			NetCDFConverterUtilities.setData1D(timeOriginalData, time1Data,
					timeDataType, nTimes, false);
			ncFileOut.write(NURCActionsIOUtils.TIME_DIM, time1Data);

			// z level Variable data
			if (hasZeta) {
				zeta1Data = NetCDFConverterUtilities.getArray(nZeta,
						zetaDataType);
				NetCDFConverterUtilities.setData1D(levelOriginalData,
						zeta1Data, zetaDataType, nZeta, false);
			...
			for (Object object : ncFileIn.getVariables()) {
				...
				if (var != null) {
					...
					if (!Double.isNaN(fV)) {
						outNoData = (float) NURCActionsIOUtils.rescaleValue(fV,
								scale, offset);
					...
					Array destArray = NetCDFConverterUtilities.getArray(
							originalVarData.getShape(), DataType.DOUBLE);
					for (int t = 0; t < nTimes; t++)
						for (int z = 0; z < nZeta; z++)
							for (int y = 0; y < nLat; y++)
								for (int x = 0; x < nLon; x++) {
									int originalValue = originalVarData
											.getInt(originalVarData.getIndex()
													.set(t, z, y, x));
									destArray
											.setDouble(
													destArray.getIndex().set(t,
															z, y, x),
													(float) ((originalValue != noData && originalValue != fillValue) ? NURCActionsIOUtils
															.rescaleValue(
																	originalValue,
																	scale,
																	offset)
