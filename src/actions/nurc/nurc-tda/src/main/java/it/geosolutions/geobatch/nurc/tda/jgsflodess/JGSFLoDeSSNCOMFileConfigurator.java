 ...
 * 
 */
public class JGSFLoDeSSNCOMFileConfigurator extends MetocConfigurationAction {
	...
	public Queue<FileSystemMonitorEvent> execute(
		...
		try {
			...
			final File ncomsDatasetDirectory = ("zip"
					...
					"NCOM", event.getSource(), tempFile) : Utilities
			...
			// ////
			File[] NCOMGridAreaFiles = ncomsDatasetDirectory
			...
			if (NCOMGridAreaFiles.length != 1) {
				if (LOGGER.isLoggable(Level.SEVERE))
					LOGGER
							.severe("Could not find NCOM Grid Area definition file.");
				throw new IOException(
						"Could not find NCOM Grid Area definition file.");
			}

			ncGridAreaDefinitionFile = NetcdfFile.open(NCOMGridAreaFiles[0]
					.getAbsolutePath());

			// input dimensions
			final Dimension xIndex = ncGridAreaDefinitionFile
					.findDimension("X_Index");

			final Dimension yIndex = ncGridAreaDefinitionFile
					.findDimension("Y_Index");

			Dimension depth = null;

			// input VARIABLES
			final Variable lonOriginalVar = ncGridAreaDefinitionFile
					.findVariable("Longitude");
			final DataType lonDataType = lonOriginalVar.getDataType();

			final Variable latOriginalVar = ncGridAreaDefinitionFile
					.findVariable("Latitude");
			...
			// - Each directory should contain separate NetCDF files for the
			// variables.
			// ////
			File[] NCOMVarFolders = ncomsDatasetDirectory
					.listFiles(new FileFilter() {
			...
			// "lscv08_MERCATOR-Forecast-T" + new Date().getTime() +
			// FilenameUtils.getBaseName(inputFileName).replaceAll("-", "") +
			// ".nc"
			// ////
			final File outputFile = new File(outDir,
					"JGSFLoDeSS_NCOM-Forecast-T"
							+ new Date().getTime()
							+ FilenameUtils.getBaseName(inputFileName)
									.replaceAll("-", "") + ".nc");
			...
			for (File NCOMVarFolder : NCOMVarFolders) {
				String[] filesList = NCOMVarFolder.list(new FilenameFilter() {
					...
					public boolean accept(File dir, String name) {
						if (FilenameUtils.getExtension(name).equalsIgnoreCase(
								"nc")
				...
				for (String fileName : filesList) {
					...
					String timeInstant = baseName.substring(baseName.length()
							- "yyyyMMdd00_hhh".length());
			...
			int t0 = Integer.parseInt(timesFound.get(0).substring(
					timesFound.get(0).lastIndexOf("_") + 1));
			int t1 = (timesFound.size() > 0 ? Integer.parseInt(timesFound
					.get(1).substring(timesFound.get(1).lastIndexOf("_") + 1))
					: t0);
			...
			for (File NCOMVarFolder : NCOMVarFolders) {
				for (File NCOMVar : NCOMVarFolder.listFiles()) {
					final String extension = FilenameUtils.getExtension(NCOMVar
							.getName());
					if (extension.equalsIgnoreCase("nc")
							|| extension.equalsIgnoreCase("netcdf")) {
						NetcdfFile ncVarFile = null;
						try {
							ncVarFile = NetcdfFile.open(NCOMVar
									.getAbsolutePath());
							if (depth == null)
							...
							for (Object obj : ncVarFile.getVariables()) {
								...
								if (!varName.equalsIgnoreCase("X_Index")
									...
									if (foundVariables.get(varName) == null) {
										...
										for (MetocElementType m : metocDictionary
												.getMetoc()) {
											if ((varName
													.equalsIgnoreCase("U_Velocity") && m
													.getName()
													.equals(
															"water velocity u-component"))
													|| (varName
															.equalsIgnoreCase("V_Velocity") && m
															.getName()
															.equals(
										...
										if (longName != null
											...
											foundVariableLongNames.put(varName,
													longName);
											foundVariableBriefNames.put(
													varName, briefName);
			...
			if (packComponents) {
				...
				if (magnitude != null) {
					...
					uom = uom.indexOf(":") > 0 ? URLDecoder.decode(uom
							.substring(uom.lastIndexOf(":") + 1), "UTF-8")
							: uom;
					if (longName != null && briefName != null) {
						foundVariables.put(magnitude.getName(), null);
						foundVariableLongNames.put(magnitude.getName(),
								longName);
						foundVariableBriefNames.put(magnitude.getName(),
								briefName);
			...
			final List<Dimension> outDimensions = NURCActionsIOUtils
					.createNetCDFCFGeodeticDimensions(ncFileOut, true,
							timesFound.size(), true, depth.getLength(),
							NURCActionsIOUtils.DOWN, true, yIndex.getLength(),
			...
			for (String varName : foundVariables.keySet()) {
				...
				if (variableBrief != null) {
					ncFileOut.addVariable(variableBrief, DataType.DOUBLE,
							outDimensions);
					ncFileOut.addVariableAttribute(variableBrief, "long_name",
							foundVariableLongNames.get(varName));
					ncFileOut.addVariableAttribute(variableBrief, "units",
					...
					if (Double.isNaN(noData)) {
						...
						if (missingValue != null) {
							noData = missingValue.getNumericValue()
									.doubleValue();
							ncFileOut.addVariableAttribute(variableBrief,
									"missing_value", noData);
							if (variableBrief.equalsIgnoreCase("watvel-v")
									|| (variableBrief
											.equalsIgnoreCase("watvel-u"))) {
								noDataWater = noData;
							}
						}
					}
			...
			if (packComponents && !Double.isNaN(noDataWater)) {
				ncFileOut.addVariableAttribute(magnitude.getBrief(),
						"missing_value", noDataWater);
			}
			...
			// time Variable data
			Array time1Data = NetCDFConverterUtilities.getArray(timesFound
					.size(), DataType.FLOAT);
			for (int t = 0; t < timesFound.size(); t++) {
				Date timeInstant = sdf.parse(timesFound.get(t));
				float timeValue = (timeInstant.getTime() - NURCActionsIOUtils.startTime) / 1000.0f;
				time1Data.setFloat(time1Data.getIndex().set(t), timeValue);
			}
			ncFileOut.write(NURCActionsIOUtils.TIME_DIM, time1Data);

			final double resY = (bbox[3] - bbox[1]) / yIndex.getLength();
			final double resX = (bbox[2] - bbox[0]) / xIndex.getLength();

			// lat Variable data
			Array lat1Data = NetCDFConverterUtilities.getArray(yIndex
					.getLength(), latDataType);
			for (int y = 0; y < yIndex.getLength(); y++)
				lat1Data.setFloat(lat1Data.getIndex().set(y),
						(float) (bbox[1] + y * resY));
			ncFileOut.write(NURCActionsIOUtils.LAT_DIM, lat1Data);

			// lon Variable data
			Array lon1Data = NetCDFConverterUtilities.getArray(xIndex
					.getLength(), lonDataType);
			for (int x = 0; x < xIndex.getLength(); x++)
				lon1Data.setFloat(lon1Data.getIndex().set(x),
						(float) (bbox[0] + x * resX));
			ncFileOut.write(NURCActionsIOUtils.LON_DIM, lon1Data);

			List<Float> depthValuesFound = new ArrayList<Float>();
			for (File NCOMVarFolder : NCOMVarFolders) {
				for (File NCOMVar : NCOMVarFolder.listFiles()) {
					if (FilenameUtils.getExtension(NCOMVar.getName())
							.equalsIgnoreCase("nc")
							|| FilenameUtils.getExtension(NCOMVar.getName())
									.equalsIgnoreCase("netcdf")) {
						NetcdfFile ncVarFile = null;
						try {
							ncVarFile = NetcdfFile.open(NCOMVar
							...
							// input dimensions
							final Dimension Depth = ncVarFile
									.findDimension("Depth");

							// input variables
							final Variable depthOriginalVar = ncVarFile
									.findVariable("Depth");
							final Array depthOriginalData = depthOriginalVar
									.read();
							...
							for (Object obj : ncVarFile.getVariables()) {
								...
								Number offset = 0.0;
								Number scale = 1.0;
								final Attribute offsetAtt = var
										.findAttribute("add_offset");
								final Attribute scaleAtt = var
								...
								if (!varName.equalsIgnoreCase("X_Index")
									...
									// //
									final int dataType;
									final DataType varDataType = var
											.getDataType();
									...
									SampleModel outSampleModel = RasterFactory
											.createBandedSampleModel(dataType, // data
																				// type
									...
									for (int z = 0; z < Depth.getLength(); z++) {
										...
										if (!depthValuesFound
												.contains(depthOriginalData
														.getFloat(depthOriginalData
																.getIndex()
																.set(z)))) {
											depthValuesFound
													.add(depthOriginalData
															.getFloat(depthOriginalData
																	.getIndex()
																	.set(z)));
										}
										...
										NURCActionsIOUtils.write2DData(
												userRaster, var,
										...
										if (LOGGER.isLoggable(Level.FINE))
											LOGGER
													.fine("Resampling to a Regular Grid ...");
										userRaster = NURCActionsIOUtils
												.warping(bbox, lonOriginalData,
														latOriginalData, xIndex
																.getLength(),
														yIndex.getLength(), 2,
														userRaster,
														(float) noData, false);
										...
										final Variable outVar = ncFileOut
												.findVariable(foundVariableBriefNames
														.get(varName));
										final Array outVarData = outVar.read();

										int tIndex = 0;
										String baseName = FilenameUtils
												.getBaseName(NCOMVar.getName());
										String timeInstant = baseName
										...
										for (int y = 0; y < yIndex.getLength(); y++)
											for (int x = 0; x < xIndex
													.getLength(); x++) {
												int originalValue = userRaster
														.getSample(x, y, 0);
												outVarData
														.setDouble(
																outVarData
																		...
																		.set(
																				...
																				y,
																				x),
																(originalValue != noData ? NURCActionsIOUtils
																		.rescaleValue(
																				originalValue,
																				scale,
																				offset)
																		: noData));
												// outVarData.setFloat(outVarData.getIndex().set(tIndex,
												// z, y, x),
												// userRaster.getSampleFloat(x,
												// y, 0));
											}
										ncFileOut.write(foundVariableBriefNames
												.get(varName), outVarData);
									}
								}
							}
						} finally {
			...
			Array zeta1Data = NetCDFConverterUtilities.getArray(depth
					.getLength(), DataType.FLOAT);
			for (int z = 0; z < depthValuesFound.size(); z++)
				zeta1Data.setFloat(zeta1Data.getIndex().set(z),
						depthValuesFound.get(z));

			ncFileOut.write(NURCActionsIOUtils.DEPTH_DIM, zeta1Data);
			if (packComponents) {
				...
				if (u != null && v != null) {
					final String magnitudeName = foundVariableBriefNames
							.get(magnitude.getName());
					final Variable magnitudeVar = ncFileOut
							.findVariable(magnitudeName);
					final Array magnitudeVarData = magnitudeVar.read();
					for (int t = 0; t < timesFound.size(); t++) {
						for (int z = 0; z < depthValuesFound.size(); z++) {
							for (int y = 0; y < yIndex.getLength(); y++) {
								for (int x = 0; x < xIndex.getLength(); x++) {
									Index index = magnitudeVarData.getIndex()
											.set(t, z, y, x);

									double uValue = u.getDouble(index);
									double vValue = v.getDouble(index);
									double magnitudeValue = (uValue != noData && vValue != noData) ? Math
											.sqrt(Math.pow(uValue, 2)
													+ Math.pow(vValue, 2))
											: noData;
									magnitudeVarData.setDouble(index,
											magnitudeValue);
								}
							}
						}
