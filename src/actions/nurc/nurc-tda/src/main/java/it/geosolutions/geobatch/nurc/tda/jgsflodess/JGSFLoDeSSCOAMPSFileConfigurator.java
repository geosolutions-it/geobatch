		...
		try {
			...
			if (fileNameFilter != null) {
				if ((filePrefix.equals(fileNameFilter) || filePrefix
						...
						&& ("zip".equalsIgnoreCase(fileSuffix) || "tar"
			...
			final File coampsDatasetDirectory = Utilities.decompress("COAMPS",
					event.getSource(), tempFile);

			// ////
			// CASE 1: A FlatFileDescriptor exists
			// - A FlatFileDescriptor is a file endings with the key word
			// "infofld".
			...
			// ////
			String[] gridInfoFileNames = coampsDatasetDirectory
					.list(new FilenameFilter() {

						public boolean accept(File dir, String name) {
							if (name.startsWith("longit")
									|| name.startsWith("latitu"))
								return true;
			...
			if (gridInfoFileNames.length < 2) {
				if (LOGGER.isLoggable(Level.SEVERE))
					LOGGER
							.severe("COAMPS grid file information could not be found!");
			...
			for (String gridInfoFileName : gridInfoFileNames) {
				if (gridInfoFileName.toLowerCase().startsWith("longit")) {
					final FlatFileGrid lonFileGrid = new FlatFileGrid(new File(
							coampsDatasetDirectory, gridInfoFileName));
				...
				if (gridInfoFileName.toLowerCase().startsWith("latitu")) {
					final FlatFileGrid latFileGrid = new FlatFileGrid(new File(
							coampsDatasetDirectory, gridInfoFileName));
			...
			File[] COAMPSFiles = coampsDatasetDirectory
			...
			for (File COAMPSFile : COAMPSFiles) {
				...
				if (COAMPSFileGrid != null
						&& COAMPSFileGrid.getWidth() == width
						&& COAMPSFileGrid.getHeight() == height) {
					final String timeInstant = COAMPSFileGrid.getTimeGroup()
							.substring(0,
									COAMPSFileGrid.getTimeGroup().length() - 2)
							+ "_"
							+ COAMPSFileGrid.getForecastTime().substring(1);
					final Long level = new Long(COAMPSFileGrid.getLevel());

					if (!varsFound.contains(COAMPSFileGrid.getParamName()
							.replaceAll("_", ""))) {
						String varName = COAMPSFileGrid.getParamName()
								.replaceAll("_", "");
						...
						for (MetocElementType m : metocDictionary.getMetoc()) {
							if ((varName.equalsIgnoreCase("lndsea") && m
									.getName().equals("land or sea"))
									|| (varName.equalsIgnoreCase("ustrue") && m
											.getName().equals(
													"wind stress u-component"))
									|| (varName.equalsIgnoreCase("vstrue") && m
											.getName().equals(
			...
			final List<Dimension> outDimensions = NURCActionsIOUtils
					.createNetCDFCFGeodeticDimensions(ncFileOut, true,
							timesFound.size(), true, levelsFound.size(),
							NURCActionsIOUtils.UP, true, height, true, width);
			...
			if (packComponents) {
				...
				String variableBrief = foundVariableBriefNames
						.get(magnitudeName);
				if (variableBrief != null) {
					ncFileOut.addVariable(variableBrief, DataType.DOUBLE,
							outDimensions);
					ncFileOut.addVariableAttribute(variableBrief, "long_name",
							foundVariableLongNames.get(magnitudeName));
					ncFileOut.addVariableAttribute(variableBrief, "units",
							foundVariableUoM.get(magnitudeName));
					ncFileOut.addVariableAttribute(variableBrief,
							"missing_value", noData);
				}
			...
			Array zeta1Data = NetCDFConverterUtilities.getArray(levelsFound
					.size(), DataType.FLOAT);
			for (int z = 0; z < levelsFound.size(); z++)
				zeta1Data.setFloat(zeta1Data.getIndex().set(z), levelsFound
						.get(z));
			ncFileOut.write(NURCActionsIOUtils.HEIGHT_DIM, zeta1Data);

			final double resY = (envelope.getMaximum(1) - envelope
					.getMinimum(1))
					/ height;
			final double resX = (envelope.getMaximum(0) - envelope
					.getMinimum(0))
					/ width;

			// lat Variable data
			Array lat1Data = NetCDFConverterUtilities.getArray(height,
					DataType.FLOAT);
			for (int y = 0; y < height; y++)
				lat1Data.setFloat(lat1Data.getIndex().set(y), (float) (envelope
						.getMinimum(1) + y * resY));
			ncFileOut.write(NURCActionsIOUtils.LAT_DIM, lat1Data);

			// lon Variable data
			Array lon1Data = NetCDFConverterUtilities.getArray(width,
					DataType.FLOAT);
			for (int x = 0; x < width; x++)
				lon1Data.setFloat(lon1Data.getIndex().set(x), (float) (envelope
						.getMinimum(0) + x * resX));
			...
			for (File COAMPSFile : COAMPSFiles) {
				...
				if (COAMPSFileGrid != null
					...
					NURCActionsIOUtils.write2DData(userRaster, COAMPSFileGrid,
							false, false);

					// Resampling to a Regular Grid ...
					if (LOGGER.isLoggable(Level.FINE))
						LOGGER.fine("Resampling to a Regular Grid ...");
					userRaster = NURCActionsIOUtils.warping(COAMPSFileGrid,
							new double[] { xmin, ymin, xmax, ymax }, lonData,
							latData, width, height, 2, userRaster, 0, false);

					final String varName = COAMPSFileGrid.getParamName()
							.replaceAll("_", "");
					...
					for (int z = 0; z < levelsFound.size(); z++)
						for (int y = 0; y < height; y++)
							for (int x = 0; x < width; x++) {
								float sample = userRaster.getSampleFloat(x, y,
										0);
								sample = (float) (sample == 0.0 ? noData
										: sample);
								outVarData.setFloat(outVarData.getIndex().set(
										tIndex, z, y, x), sample);
							}
