 ...
 * 
 */
public class INGVFileConfiguratorAction extends MetocConfigurationAction {
		...
		try {
			...
			final List<Dimension> outDimensions = NURCActionsIOUtils
					.createNetCDFCFGeodeticDimensions(ncFileOut, true, nTimes,
							hasDepth, nDepths, NURCActionsIOUtils.DOWN, true,
							nLat, true, nLon);
			...
			for (String varName : foundVariables.keySet()) {
				boolean hasLocalDepth = NetCDFConverterUtilities
						.hasThisDimension(foundVariables.get(varName),
								depthName);
			...
			final Date timeOriginDate = toSdf.parse(timeOriginalVar
					.findAttribute("time_origin").getStringValue().trim()
					.toLowerCase());
			int TAU = 0;
		...
		if (referenceTime != null && forecastDate != null) {
			...
			if (forecastDays != null) {
				...
				if (index != -1) {
					...
					millisFromStartDate = calendar.getTimeInMillis()
							- startTime;
				}
			} else
				throw new IllegalArgumentException(
						"Unable to find forecast day");
		}
