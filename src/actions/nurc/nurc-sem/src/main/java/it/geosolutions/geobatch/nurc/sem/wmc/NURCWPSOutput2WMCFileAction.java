...
public class NURCWPSOutput2WMCFileAction extends
	...
	private final SimpleDateFormat sdf = new SimpleDateFormat(
			"yyyyMMdd'T'HHmmss'Z'");
	private final SimpleDateFormat wpssdf = new SimpleDateFormat(
			"yyyyMMdd'T'HHmmss.SSS'Z'");

	public static final long matLabStartTime;

	static {
		GregorianCalendar calendar = new GregorianCalendar(0000, 00, 01, 00,
				00, 00);
		calendar.setTimeZone(NURCActionsIOUtils.UTC);
		matLabStartTime = calendar.getTimeInMillis();
	}

	protected NURCWPSOutput2WMCFileAction(
			GeoServerActionConfiguration configuration) throws IOException {
		super(configuration);
		sdf.setTimeZone(NURCActionsIOUtils.UTC);
		wpssdf.setTimeZone(NURCActionsIOUtils.UTC);
	}
	...
	public Queue<FileSystemMonitorEvent> execute(
		...
		try {
			...
			File outDir = null;
			if (packData) {
				outDir = Utilities.createTodayDirectory(workingDir,
						inputBaseName + "packed", true);
			} else
				outDir = Utilities.createTodayDirectory(workingDir,
						inputBaseName, true);
			...
			if (packData) {
				...
				final File tempFile = File
						.createTempFile(inputBaseName, ".tmp");
				if (inputFileName.contains("TDA"))
					isTDA = true;
				final File wpsDatasetDirectory = Utilities.decompress("WPSOUT",
						event.getSource(), tempFile);

				final File[] wpsFiles = wpsDatasetDirectory
				...
				for (File wpsFile : wpsFiles) {
					...
					final String time = name.substring(11, name
							.indexOf("_", 12));
				...
				for (Long timeInstant : timesMap.keySet()) {
					...
					final String extension = FilenameUtils.getExtension(wpsFile
							.getName());
					final String wpsFileName = wpsFile.getAbsolutePath();
					if (extension.equalsIgnoreCase("nc")
						...
						try {
							...
							if (!initialized) {
								...
								initialized = true;
								final Dimension lat = ncVarFile
										.findDimension("lat");
								final Dimension lon = ncVarFile
										.findDimension("lon");
								nLat = lat.getLength();
								nLon = lon.getLength();
								...
								final List<Dimension> outDimensions = NURCActionsIOUtils
										.createNetCDFCFGeodeticDimensions(
												ncFileOut, true, times, false,
												0, "", true, nLat, true, nLon,
												DataType.INT);
								for (Object obj : ncVarFile.getVariables()) {
									...
									if (!varName.equalsIgnoreCase("lat")
										...
										final String shortName = varName
												.replace(" ", "").toLowerCase();
										ncFileOut.addVariable(shortName,
								...
								final Array latOriginalData = latOriginalVar
										.read();
								final Array lonOriginalData = lonOriginalVar
										.read();

								Array timeData = new ArrayLong(
										new int[] { times });
								int i = 0;
								for (Long timeI : timesMap.keySet()) {
									long timeValue = (timeI - NURCActionsIOUtils.startTime) / 1000;
									timeData.setLong(timeData.getIndex().set(
											i++), timeValue);
								}
								ncFileOut.create();

								ncFileOut.write(NURCActionsIOUtils.LAT_DIM,
										latOriginalData);
								ncFileOut.write(NURCActionsIOUtils.LON_DIM,
										lonOriginalData);
								ncFileOut.write(NURCActionsIOUtils.TIME_DIM,
										timeData);
							}
							...
							for (Object obj : ncVarFile.getVariables()) {
								...
								if (!varName.equalsIgnoreCase("lat")
									...
									final Variable outVar = ncFileOut
											.findVariable(shortName);
									final Array outData = outVar.read();
									final Array inData = var.read();
									for (int y = 0; y < nLat; y++) {
										for (int x = 0; x < nLon; x++) {
											Index index = outData.getIndex()
													.set(timeIndex, y, x);
											Index inIndex = inData.getIndex()
													.set(y, x);
											outData.setDouble(index, inData
													.getDouble(inIndex));
										}
									}
			...
			final Dimension timeDim = ncFileIn
					.findDimension(NURCActionsIOUtils.TIME_DIM);
			final boolean timeDimExists = timeDim != null;

			final Dimension depthDim = ncFileIn
					.findDimension(NURCActionsIOUtils.DEPTH_DIM);
			final boolean depthDimExists = depthDim != null;

			final Dimension heightDim = ncFileIn
					.findDimension(NURCActionsIOUtils.HEIGHT_DIM);
			final boolean heightDimExists = heightDim != null;

			Dimension latDim = ncFileIn
					.findDimension(NURCActionsIOUtils.LAT_DIM);
			if (latDim == null)
				latDim = ncFileIn
						.findDimension(NURCActionsIOUtils.LAT_DIM_LONG);
			final boolean latDimExists = latDim != null;

			Dimension lonDim = ncFileIn
					.findDimension(NURCActionsIOUtils.LON_DIM);
			if (lonDim == null)
				lonDim = ncFileIn
						.findDimension(NURCActionsIOUtils.LON_DIM_LONG);
			...
			final Variable timeOriginalVar = ncFileIn
					.findVariable(NURCActionsIOUtils.TIME_DIM);
			final Variable yearOriginalVar = ncFileIn
					.findVariable(NURCActionsIOUtils.TIME_YEAR);
			final Variable monthOriginalVar = ncFileIn
					.findVariable(NURCActionsIOUtils.TIME_MONTH);
			final Variable dayOriginalVar = ncFileIn
					.findVariable(NURCActionsIOUtils.TIME_DAY);
			final Variable hourOriginalVar = ncFileIn
					.findVariable(NURCActionsIOUtils.TIME_HOUR);
			...
			timeCalendar.setTimeZone(NURCActionsIOUtils.UTC);
			...
			if (!hasTime) {
				...
				if (isTDA) {
					baseTime = sdf.format(timeOriginalData
							.getLong(timeOriginalIndex.set(0))
							* 1000 + NURCActionsIOUtils.startTime);
				} else {
					timeCalendar.set(yearOriginalData.getInt(0),
							monthOriginalData.getInt(0) - 1, dayOriginalData
									.getInt(0), hourOriginalData.getInt(0), 0,
							0);
			...
			Variable lonOriginalVar = ncFileIn
					.findVariable(NURCActionsIOUtils.LON_DIM);
			if (lonOriginalVar == null)
				lonOriginalVar = ncFileIn
						.findVariable(NURCActionsIOUtils.LON_DIM_LONG);

			Variable latOriginalVar = ncFileIn
					.findVariable(NURCActionsIOUtils.LAT_DIM);
			if (latOriginalVar == null)
				latOriginalVar = ncFileIn
						.findVariable(NURCActionsIOUtils.LAT_DIM_LONG);
			...
			if (hasZeta) {
				zetaOriginalVar = ncFileIn
						.findVariable(depthDimExists ? NURCActionsIOUtils.DEPTH_DIM
								: NURCActionsIOUtils.HEIGHT_DIM);
			...
			for (Variable var : foundVariables) {
				if (var != null) {
					...
					if (var.getRank() == 1
							|| varName
									.equalsIgnoreCase(NURCActionsIOUtils.LAT_DIM)
							|| varName
									.equalsIgnoreCase(NURCActionsIOUtils.LON_DIM)
							|| varName
									.equalsIgnoreCase(NURCActionsIOUtils.LAT_DIM_LONG)
							|| varName
									.equalsIgnoreCase(NURCActionsIOUtils.LON_DIM_LONG)
							|| varName
									.equalsIgnoreCase(NURCActionsIOUtils.TIME_DIM)
							|| varName
									.equalsIgnoreCase(NURCActionsIOUtils.HEIGHT_DIM)
							|| varName
									.equalsIgnoreCase(NURCActionsIOUtils.DEPTH_DIM))
						continue;
					...
					if (canProceed) {
						...
						final boolean hasLocalZLevel = NetCDFConverterUtilities
								.hasThisDimension(var,
										NURCActionsIOUtils.DEPTH_DIM)
								|| NetCDFConverterUtilities.hasThisDimension(
										var, NURCActionsIOUtils.HEIGHT_DIM);
						final boolean hasLocalTime = NetCDFConverterUtilities
								.hasThisDimension(var,
										NURCActionsIOUtils.TIME_DIM)
								&& hasTime;
						...
						final String variableName = varName.replace("_", "")
								.replace(" ", "");
						final ArrayList<String> ranges = new ArrayList<String>(
								(hasLocalTime ? nTime : 1)
										* (hasLocalZLevel ? nZeta : 1));
						...
						for (int t = 0; t < (hasLocalTime ? nTime : 1); t++) {
							for (int z = 0; z < (hasLocalZLevel ? nZeta : 1); z++) {
								...
								final Array minMaxArray = NURCActionsIOUtils
										.write2DData(userRaster, var,
												originalVarArray, true, true,
												dimArray, true);
								...
								String refZeta = hasLocalZLevel ? elevLevelFormat(zetaOriginalData
										.getDouble(zetaOriginalData.getIndex()
												.set(z)))
										: "0000.000";
								final StringBuilder coverageName = new StringBuilder(
										inputBaseName).append("_").append(
										variableName).append("_").append(
										refZeta).append("_").append(refZeta)
										.append("_");
								String refTime = null;
								if (!hasTime) {
									...
									if (isTDA) {
										...
										refTime = timeDimExists ? sdf
												.format(NURCActionsIOUtils.startTime
														+ timeOriginalData
																.getLong(timeOriginalIndex
																		.set(t))
														* 1000)
												: "00000000T000000Z";
										coverageName.append(refTime);
									} else {
										coverageName.append(baseTime).append(
												"_");
											...
											timeCalendar
													.set(
															...
															hourOriginalData
																	.getInt(t),
															0, 0);
										String ftime = timeDimExists ? sdf
								...
								ranges.add(new StringBuilder(refTime).append(
										",").append(refZeta).append(",")
										.append(minMaxArray.getDouble(0))
										.append(",").append(
												minMaxArray.getDouble(1))
										.append("\n").toString());
								coverageName.append("-T").append(
										System.currentTimeMillis());
								final String nd = Double.isNaN(noData) ? "-9999.0"
										: Double.toString(noData);
								coverageName.append("_").append(nd);

								File gtiffFile = Utilities
										.storeCoverageAsGeoTIFF(gtiffOutputDir,
												coverageName.toString(),
												variableName, userRaster,
												noData, envelope,
						...
						try {
							outInfoFile = new File(
									gtiffOutputDir,
									new StringBuilder(inputBaseName)
											.append("_").append(variableName)
											.toString()
											+ WMCFileConfiguratorAction.INFO_EXTENSION);
							BufferedWriter writer = new BufferedWriter(
									new FileWriter(outInfoFile));
