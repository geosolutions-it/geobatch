 ...
 * 
 */
public class NRLNCOMFileConfiguratorAction extends NURCSEMBaseConfiguratorAction {
	...
	protected NRLNCOMFileConfiguratorAction(MetocActionConfiguration configuration) throws IOException {
		super(configuration);
	}
	...
	protected void writeDownNetCDF(File outDir, String inputFileName)
		...
		double[] bbox = NURCActionsIOUtils.computeExtrema(latOriginalData, lonOriginalData, lat_dim, lon_dim);
		...
		outputFile = new File(outDir, "lscv08_NCOM"
				+ (inputFileName.contains("nest") ? "nest"
						+ inputFileName.substring(inputFileName
								.indexOf("nest")
		...
		final List<Dimension> outDimensions = NURCActionsIOUtils.createNetCDFCFGeodeticDimensions(ncFileOut, true, time_dim.getLength(), hasDepth, hasDepth ? depth_dim.getLength() : 0, NURCActionsIOUtils.DOWN, true, lat_dim.getLength(), true, lon_dim.getLength());
						...
						for (int x = 0; x < lon_dim.getLength(); x++) {
							if (!hasDepth) {
								...
								destArray.setDouble(destArray.getIndex().set(t,y, x),
										(originalValue != noData ? (originalValue * scale) + offset : noData));
							} else {
