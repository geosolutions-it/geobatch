 ...
 * 
 */
public class NRLNCOMFileConfiguratorAction extends MetocConfigurationAction {
		...
		try {
			...
			final List<Dimension> outDimensions = NURCActionsIOUtils
					.createNetCDFCFGeodeticDimensions(ncFileOut, true, time_dim
							...
							.getLength() : 0, NURCActionsIOUtils.DOWN, true,
			...
			if (hasDepth)
				ncFileOut
						.write(NURCActionsIOUtils.DEPTH_DIM, depthOriginalData);
			...
			for (String varName : foundVariables.keySet()) {
				...
				ncFileOut
						.write(foundVariableBriefNames.get(varName), destArray);
			}
