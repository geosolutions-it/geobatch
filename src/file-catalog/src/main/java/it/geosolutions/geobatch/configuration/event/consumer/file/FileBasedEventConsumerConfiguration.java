 ...
 * 
 * <P>
 * TODO: we may need another hierarchy level for a
 * <B>BaseEventConsumerConfiguration</B> class
 * 
 * @author Simone Giannecchini, GeoSolutions
 */
public class FileBasedEventConsumerConfiguration extends BaseConfiguration
		implements EventConsumerConfiguration {

	/**
	 * List of configurable actions that will be sequentially performed at the
	 * end of event consumption.
	 */
	...
	/**
	 * The configuring directory. This is the directory where the consumer will
	 ...
	 * They are needed for a post-load binding; loader logic will put the proper
	...
	public FileBasedEventConsumerConfiguration clone() { // throws
		...
		// clone its elements
		final List<FileEventRule> clonedRules = new ArrayList<FileEventRule>(
				rules.size());
		for (FileEventRule rule : rules)
			clonedRules.add(rule.clone());
		object.setRules(clonedRules);

		final List<ActionConfiguration> clonedActions = new ArrayList<ActionConfiguration>(
				actions.size());
