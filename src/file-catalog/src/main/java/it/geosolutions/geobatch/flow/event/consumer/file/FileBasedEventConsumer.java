...
public class FileBasedEventConsumer
		extends
		BaseEventConsumer<FileSystemMonitorEvent, FileBasedEventConsumerConfiguration>
		implements
	...
	private final static Logger LOGGER = Logger
			.getLogger(FileBasedEventConsumer.class.toString());

	// ----------------------------------------------- PUBLIC CONSTRUCTORS
	public FileBasedEventConsumer(
			FileBasedEventConsumerConfiguration configuration)
			throws InterruptedException, IOException {

		final File catalogFile = new File(((FileBaseCatalog) CatalogHolder
				.getCatalog()).getBaseDirectory());
		final File workingDir = IOUtils.findLocation(configuration
				.getWorkingDirectory(), catalogFile);
	...
	// -------------------------------------------------------------------------
	/**
	 * This method allows the BaseDispatcher to check if an Event can be
	 * processed by the current BaseEventConsumer.
	 * 
	 * @return boolean true if the event can be accepted, i.e. this
	 *         BaseEventConsumer was waiting for it.
	...
	private boolean canConsume(FileSystemMonitorEvent event) {
		...
		boolean res = this.checkRuleConsistency(event.getNotification(),
				filePrefix, fileName, true);

		// check optinal rules if needed
		if (!res) {
			res = this.checkRuleConsistency(event.getNotification(),
					filePrefix, fileName, false);
		}
	...
	/**
	 * Check if all queued events refer to files in the same dir. CHECKME: is it
	 * really needed? will it not break flows that need scattered files?
	 */
	private boolean checkSamePath(String fullpath) {
		for (FileSystemMonitorEvent event : eventsQueue) {
			String existingFP = FilenameUtils.getFullPath(event.getSource()
					.getAbsolutePath());
	...
	private void initialize(FileBasedEventConsumerConfiguration configuration,
			File workingDir) throws InterruptedException {
		...
		for (ProgressListenerConfiguration plConfig : configuration
			...
			final ProgressListenerService progressListenerService = CatalogHolder
					.getCatalog().getResource(serviceID,
			...
			if (progressListenerService != null) {
				...
				throw new IllegalArgumentException("Could not find '"
						+ serviceID + "' listener, declared in "
		...
		for (ActionConfiguration actionConfig : configuration.getActions()) {
			...
			final ActionService<FileSystemMonitorEvent, ActionConfiguration> actionService = CatalogHolder
					.getCatalog().getResource(actionServiceID,
							ActionService.class);
			if (actionService != null) {
				Action<FileSystemMonitorEvent> action = actionService
						.createAction(actionConfig);
				if (action == null) {
					throw new IllegalArgumentException(
							"Action could not be created for config "
									+ actionConfig);
				}
				...
				for (ProgressListenerConfiguration plConfig : actionConfig
					...
					if (progressListenerService != null) {
						...
						throw new IllegalArgumentException("Could not find '"
								+ listenerServiceID + "' listener,"
								+ " declared in " + actionConfig.getId()
				...
				throw new IllegalArgumentException("ActionService not found '"
						+ actionServiceID + "' for ActionConfig '"
	...
	public void run() {
		...
		try {
			...
			for (FileSystemMonitorEvent ev : this.eventsQueue) {
				if (LOGGER.isLoggable(Level.INFO))
					LOGGER
							.info(new StringBuilder("FileBasedEventConsumer [")
				...
				final String fileBareName = FilenameUtils
						.getName(sourceDataFile.toString());
				...
				final File destDataFile = new File(currentRunDirectory,
						fileBareName);
				...
				if (IOUtils.acquireLock(this, sourceDataFile)) {
					...
					LOGGER.info(new StringBuilder("FileBasedEventConsumer [")
							.append(Thread.currentThread().getName()).append(
									"]: accepted file ").append(sourceDataFile)
							.toString());
				} else {
					LOGGER.severe(new StringBuilder("FileBasedEventConsumer [")
							.append(Thread.currentThread().getName()).append(
									"]: could not lock file ").append(
			...
			LOGGER.info(new StringBuilder("FileBasedEventConsumer [").append(
					...
					"]: new element processed.").toString());
			...
			if (LOGGER.isLoggable(Level.SEVERE)) {
				LOGGER.log(Level.SEVERE, "FileBasedEventConsumer "
						+ Thread.currentThread().getName() + " Error during "
						+ e.getAction().getClass().getSimpleName()
	...
	protected void performBackup(final File sourceDataFile, File backup,
			...
			if (IOUtils.acquireLock(this, sourceDataFile)) {
				...
				if (LOGGER.isLoggable(Level.SEVERE)) {
					LOGGER.severe(Thread.currentThread().getName()
							+ " - Can't lock file " + sourceDataFile);
				}
			}
		} catch (IOException e) {
			if (LOGGER.isLoggable(Level.SEVERE)) {
	...
	public boolean consume(FileSystemMonitorEvent event) {
		if (getStatus() != EventConsumerStatus.IDLE
				&& getStatus() != EventConsumerStatus.WAITING) {
			return false;
		}
		if (!canConsume(event)) {
			return false;
	...
	public String toString() {
		return getClass().getSimpleName()
				...
				+ numInputFiles
				+ (isPaused() ? " PAUSED" : "")
				+ (eventsQueue.isEmpty() ? "" : (" first event:" + eventsQueue
						.peek().getSource().getName())) + "]";
	}
}
