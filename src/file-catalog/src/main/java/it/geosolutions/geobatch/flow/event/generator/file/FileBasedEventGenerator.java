...
public class FileBasedEventGenerator<T extends EventObject> extends BaseEventGenerator<T> {
     ...
     * Constructor
     * 
     * @param configuration the FileBasedEventGeneratorConfiguration
     *            configuration
     * @throws IOException see initialize
     * @thrown NullPointerException see initialize
     */
    public FileBasedEventGenerator(FileBasedEventGeneratorConfiguration configuration) throws IOException,
     ...
     * @param configuration
     */
    private void initialize(final FileBasedEventGeneratorConfiguration configuration) {
         ...
         */
            ...
            throw new IllegalArgumentException(
                                               "Unable to initialize FileBasedEventGenerator using the configured watchDirectory: "
                                                   + configuration.getWatchDirectory());
        } else {
            // really important otherwise GEOBATCH_CONFIG_DIR contents will be
            // erased!!!
            this.keepFiles = true;
            this.watchDirectory = watchDirectory = ((FileBaseCatalog)CatalogHolder.getCatalog())
                .getConfigDirectory();
        }
        // WORKAROUND
        ...
        if (this.watchDirectory.isDirectory() && this.watchDirectory.exists()
            ...
            this.fsMonitor = FSMSPIFinder.getMonitor(params, configuration.getOsType(),
    ...
    private final class GBEventListener implements FileSystemListener {
        ...
        public void onFileSystemEvent(final FileSystemEvent fe) {
            if ((fe != null) && (fe.getSource() != null)) {
                ...
                final FileSystemEventType acceptedEvent = FileBasedEventGenerator.this.getEventType();

                final FileSystemEventType incomingEvent = fe.getEventType();

                if (LOGGER.isInfoEnabled()) {
                    LOGGER.info(new StringBuilder("Event: ").append(incomingEvent.toString()).append(" ")
