    ...
    implements FlowManager<FileSystemEvent, FileBasedFlowConfiguration>, Runnable, Job
{
     ...
     * @throws IOException
     */
    public FileBasedFlowManager(FileBasedFlowConfiguration configuration) throws IOException, NullPointerException
    {
        ...
        try
        {
            ...
            if (workingDirectory == null)
            {
                throw new IllegalArgumentException(new StringBuilder(
                        "Working dir is invalid: ").append('>').append(baseDir).append("< ").append('>').append(configuration.getWorkingDirectory()).append("< ").toString());
            }
            ...
            final int queueSize = (configuration.getWorkQueueSize() > 0) ? configuration.getWorkQueueSize() : fs.getWorkQueueSize();
            final int corePoolSize = (configuration.getCorePoolSize() > 0) ? configuration.getCorePoolSize() : fs.getCorePoolSize();
            final int maximumPoolSize = (configuration.getMaximumPoolSize() > 0) ? configuration.getMaximumPoolSize() : fs.getMaximumPoolSize();
            final long keepAlive = (configuration.getKeepAliveTime() > 0) ? configuration.getKeepAliveTime() : fs.getKeepAliveTime(); // seconds

            final BlockingQueue<Runnable> queue = new ArrayBlockingQueue<Runnable>(
                    queueSize);

            this.executor = new ThreadPoolExecutor(corePoolSize,
                    maximumPoolSize, keepAlive, TimeUnit.SECONDS, queue);
    ...
    public void dispose(FileBasedEventConsumer fbec)
    {
        if (!eventConsumers.contains(fbec))
        {
            throw new IllegalArgumentException(
                "FileBasedFlowManager:dispose(): This flow is not managing " +
                fbec);
        }
    ...
    public synchronized void run()
    {
        for (;;)
        {
            ...
            while (paused)
            {
                ...
                {
                    if (initialized &&
                            ((eventGenerator != null) && eventGenerator.isRunning()))
                    {
            ...
            {
                // Initialize objects
                this.dispatcher = new FileBasedEventDispatcher(this,
                        eventMailBox);
                dispatcher.start();
                initialized = true;
            }
    ...
    private void createGenerator()
    {
        ...
        final EventGeneratorService<FileSystemEvent, EventGeneratorConfiguration> generatorService =
            CatalogHolder.getCatalog().getResource(serviceID,
    ...
    public synchronized void resume()
    {
        ...
        if (LOGGER.isInfoEnabled())
        {
            LOGGER.info("FileBasedFlowManager::resume(): RESUMING ->" +
                this.getId());
        }
        ...
        if (!started)
        {
            ...
            if (LOGGER.isInfoEnabled())
            {
                LOGGER.info("FileBasedFlowManager::resume(): STARTED ->" +
                    this.getId());
            }
        }
        else if (!isRunning())
        {
            ...
            if (LOGGER.isInfoEnabled())
            {
                LOGGER.info("FileBasedFlowManager::resume(): RESUMED ->" +
                    this.getId());
            }
        }
    ...
    public synchronized boolean pause()
    {
        if (LOGGER.isInfoEnabled())
        {
            LOGGER.info("FileBasedFlowManager::pause(): PAUSING -> " +
                this.getId());
        }
    ...
    Future<Queue<FileSystemEvent>> execute(FileBasedEventConsumer consumer)
    {
        ...
        {
            final String message = "FileBasedFlowManager:execute(): Consumer " +
                consumer + " is not in an EXECUTING state.";
            if (LOGGER.isErrorEnabled())
            {
                LOGGER.error(message);
            }
        ...
        {
            final String message = "FileBasedFlowManager:execute(): Consumer " +
                consumer + " is not handled by the current flow manager.";
            if (LOGGER.isErrorEnabled())
            {
                LOGGER.error(message);
            }
    ...
    public void postEvent(FileSystemEvent event)
    {
        ...
        catch (InterruptedException e)
        {
            ...
            {
                LOGGER.error(
                    "FileBasedFlowManager:postEvent(): Unable to add event [" +
                    event.toString() +
                    "] to the flow manager (id:" + this.getId() +
                    ") eventMailBox.\nMessage is:" +
                    e.getLocalizedMessage(), e);
